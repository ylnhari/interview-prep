...
- `network-layers` — all seven layers of the reference model in one column, each with what it is responsible for, the four-layer model that is actually implemented bracketed beside it, and layers 3, 4 and 7 marked as the only ones anyone names; a message flows down the left edge. Fits: the layer model, what layer 4 and layer 7 can see, OSI versus TCP/IP, why a balancer's layer decides what it can route on.
- `tcp-handshake` — the three-message handshake as a ladder between client and server with the one wasted round trip bracketed, beside a bar chart of round trips before the first byte for TCP, TLS 1.2, TLS 1.3 and QUIC. Fits: connection setup cost, why a first request is slow, TLS handshake cost, what QUIC saves.
