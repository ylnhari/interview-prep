...
Every term that points at a concept the reader has not met must be explained where it is used, or the chapter that explains it must be named. This covers abbreviations, initialisms and notation, not only ideas: SLO, TTL, CAP, PACELC, LRU, NDCG, PII, p99, N+1 and O(1) all have to be expanded at first use in the chapter that uses them. A concept is taught in exactly one chapter and referenced from every other by link; repeating the same tutorial in several chapters is the fault to avoid. Teach it whole in that one place: a section that names a family of things explains the whole family, so a reader who meets one member does not leave wondering about the rest. Links are section-level deep links, `#<chapter-id>/<section-id>`, so the reader lands on the explanation rather than the top of a chapter. Never explain a concept by halves. A partial explanation dropped inline, because the term happens to be needed a few sections early, leaves the reader with a fragment and no way to complete it. The choice is between naming and teaching, and only two moves are allowed. **Name it**: when the reader only needs to know what the letters stand for, expand it in a few words and link to where it is taught. **Teach it**: when the reader needs to understand it, write one comprehensive section with a diagram in the chapter that owns the subject, and link to that section from every other mention. If no chapter owns it yet, the fix is to write that section, not to sprinkle explanations at each point of use. Three ways to resolve a short form, in order of preference: expand it in a single clause where it first appears; expand it and link to the chapter that teaches it, using an inline deep link such as `<a href="#sre-practices">Site reliability practices</a>`; or, when the term runs through the whole chapter, give it an entry in that chapter's Key terms. Product and library names, SQL keywords and HTTP methods are names rather than abbreviations and need no expansion, though a method like PATCH still needs one clause saying what it does. `node local/tools/abbrev-audit.js` checks this mechanically and must come back clean apart from documented false positives. Writing "layer 4" without first saying that network software is described in layers, that layer 4 is the transport layer of addresses and ports and layer 7 is the application layer where the request itself is visible, leaves the term pointing at nothing.

A concept is taught in exactly one chapter, and referenced from everywhere else by a link. The chapter that teaches it carries the full explanation; every other chapter names the distinction it is using in a clause and links to that chapter, with a section-level deep link where the explanation lives in one section: `<a href="#networking-basics/nb-layers">the network layers</a>`. Repeating the same tutorial in several chapters is the thing to avoid. It is not thoroughness: it goes stale in different ways in each copy, it pads chapters with material that is not their subject, and it leaves the reader unsure which version is the real one. Network layers are taught in `networking-basics`; if a new chapter needs a concept that is already taught elsewhere, link to it rather than explaining it again.
