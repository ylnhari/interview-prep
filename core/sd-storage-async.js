...
<p><b>How the three differ, in one comparison.</b> A task queue delivers one message to exactly one consumer, and the message is gone once that consumer acknowledges it. An event stream appends one message to a durable log that stays for a retention period, so several independent consumer groups can each read all of it at their own pace and any of them can go back and read it again. Pub/sub delivers one message to every subscriber at once, and in its plain form keeps nothing: a subscriber that was not connected when the message was published never sees it.</p>
