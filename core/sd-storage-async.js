...
(function (root) {
  ...
  root.PREP_CORE['storage-engines'] = {
    ...
    why: `Any interview that goes past "design a database" or "pick a data store for this" eventually asks how the storage layer actually works underneath the query language, and naming a product without saying why it fits falls apart under one follow-up question. Interviewers are listening for whether you understand the one choice that explains almost everything else: rewriting data in place, or only ever appending it and cleaning up later. Another recurring question is where to keep large media files, backups, or bulk exports that a general-purpose database is not built to serve; object storage fits those workloads for reasons worth stating precisely rather than by name alone.`,
    learn: [
      {
        id: 'se-0',
        part: 'field',
...
<p>Left alone, this process produces an ever-growing pile of SSTables, each of which a read might have to check. <b>Compaction</b> merges SSTables together in the background, much like Bitcask's merge, discarding any key value that a newer SSTable has since overwritten. In one common leveled-compaction design, SSTables are organised into <b>levels</b>: level 0 holds freshly flushed files and may have overlapping key ranges; deeper levels hold larger files with non-overlapping key ranges, and each level has a size target larger than the one above it. Compacting files into the next level bounds the number of files a read has to check. Other compaction strategies use different layouts.</p>`,
...
<p>The checksum earns its place on every read, not just at write time. Recomputing it over the bytes just read and comparing it to the stored value catches disk corruption and torn writes. During recovery, an incomplete unacknowledged tail record can be discarded once the parser proves it is only the final partial append. A checksum failure in an acknowledged or previously valid record is different: the engine must surface corruption and recover from another replica, backup, or verified copy where possible. Treating it as "never written" would silently turn corruption into data loss. Checking the comparison on every read is what end-to-end integrity means.</p>`,
      ...
      },
      {
        ...
        body: `<p>Object storage systems like Amazon S3 solve a different problem than a database: store an enormous number of large, whole, immutable blobs cheaply and durably, under a flat namespace of keys, rather than mutable rows a query can filter, join, or partially update. The following is one simplified illustrative architecture; actual providers use different internal designs.</p>
...
If this were instead one job assigned to one competing worker per delivery - say, just sending the push notification, with nothing else consuming the event - I'd lean toward RabbitMQ or SQS because a task queue is simpler for that model. I would still make the send idempotent because an acknowledgement failure can cause redelivery. The deciding factor is the number of independent parties that need the event and whether replay matters, not raw scale."`
      },
      {
        id: 'qs-a3',
        type: 'drill',
