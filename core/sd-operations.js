...
<li><b>Guardrail.</b> A check on a language model's input or output - for PII (personally identifiable information, anything that can be traced back to a named person; see <a href="#sre-practices">Site reliability practices</a>), injection, policy violations, or unsupported claims - separate from the model generating the answer.</li>
...
<p><b>High-level design, in brief.</b> The client uploads in chunks directly to object storage using a resumable upload protocol. Completion of the upload emits an event that triggers a transcoding pipeline - an orchestrated set of jobs, not one long script - producing several resolutions and bitrates plus a manifest for adaptive bitrate streaming. Finished outputs go back into object storage behind a CDN (content delivery network, a set of edge servers near the viewer that cache content so it does not have to travel back to the origin on every request; see <a href="#caching">Caching</a>) for playback.</p>
