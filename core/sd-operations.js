...
<li><b>SLO (service level objective).</b> A target for an SLI over a stated window, such as "99.9% of requests complete under 200ms over 30 days." An SLO may use a fraction, threshold, percentile, or another clearly defined indicator.</li>
...
<li><b>Progressive delivery.</b> Releasing exposure in measured stages with a check at each step, as in a canary or staged traffic shift. Blue-green is progressive only when traffic is shifted between its environments in stages.</li>
...
<p>The four numbers worth a dashboard of their own are the <b>golden signals</b>: <b>latency</b> (how long a request takes, always at a percentile), <b>traffic</b> (how much load is arriving), <b>errors</b> (how often it fails), and <b>saturation</b> (how full the most constrained resource is). Almost every incident shows up in at least one of these before a customer files a complaint.</p>`,
      ...
      },
      {
        ...
        body: `<p>An <b>SLI</b> is a number you actually measure: the fraction of requests served under 200 milliseconds, say. An <b>SLO</b> is the target for that indicator over a stated window - "99.9% of requests under 200ms over 30 days." It may be expressed as a fraction, threshold, percentile, or another defined measure; a percentile is not required. An <b>SLA</b> attaches contractual consequences to a service commitment; most internal targets are objectives, not agreements, and mixing up the two words in front of someone who lives by them is a small tell.</p>
...
<p>The <b>multi-region</b> choice trades cost against RTO. <b>Active-passive</b> keeps a standby region ready but idle - cheaper to run, slower to bring fully up to speed and traffic. <b>Active-active</b> serves real traffic from more than one region at once, so failover is a traffic-routing change rather than a cold start - a lower RTO, at the cost of running (and keeping consistent) full capacity in more than one place all the time.</p>`,
        deeper: `<p>Worth naming the mechanics behind "backup": a <b>full backup</b> copies everything; an <b>incremental backup</b> copies only what changed since the previous backup of any kind, so it is the smallest and fastest to take but a restore has to replay the whole chain since the last full one; a <b>differential backup</b> copies everything that changed since the last full backup, so each one grows larger than the last but a restore needs only two pieces, the full backup and the newest differential; and a storage-level <b>snapshot</b> is a point-in-time copy taken by the underlying disk or database engine, usually the fastest of all to take and to restore. The trade running through those three is the same one every time: less time and space spent taking the backup means more time and more moving parts during the restore. A widely cited rule of thumb, the <b>3-2-1 rule</b>, says to keep three copies of data, on two different kinds of storage, with one copy off-site - aimed squarely at the failure mode where the backup and the primary die together, in the same data centre, from the same cause.</p>`,
        check: {
...
<p><b>Encryption at rest</b> protects stored data if the underlying disk or backup is ever accessed directly; <b>encryption in transit</b> protects data moving between services or to a client from being read or altered on the network. Neither substitutes for access control - both assume the data is unreadable without the key, not that nobody can reach it. An <b>audit log</b> is a separate, append-only, tamper-resistant record of who accessed or changed what and when, kept specifically so a security review or an investigation has an answer that does not depend on trusting the same system that might have been compromised.</p>`,
        deeper: `<p>A genuine tension worth raising yourself: an erasure request and an audit log, or a financial ledger designed to be append-only, can pull in opposite directions. The permitted response depends on the jurisdiction, retention duty, legal basis, and data category; it is not a universal rule that the event may always be kept. A common technical pattern, when policy and law permit it, is to remove or irreversibly anonymise personal fields while retaining the minimum required event, and to let encrypted backups expire under a documented retention schedule rather than selectively rewriting them. Legal and privacy owners must define the actual obligation.</p>`,
        check: {
...
<p><b>OAuth</b>, specifically the authorization code flow most web and mobile apps use, lets a user grant one application limited access to data on another service without handing over a password. The application redirects the user to the authorization server; after authentication and consent, that server redirects back with a short-lived, single-use <b>authorization code</b>. The application's backend exchanges that code at the token endpoint for a scoped access token, then presents the token on API calls. Public clients such as mobile and browser applications use <b>PKCE</b> so an intercepted code cannot be redeemed without the original verifier. Scopes limit what the token can do, and access can be revoked without changing the user's password.</p>
...
<p>An interviewer expects working knowledge of the common attacks and their standard defences: <b>SQL injection</b> (untrusted input reaching a query unescaped; fixed with parameterised queries, never string concatenation), <b>cross-site scripting</b> (untrusted input rendered as executable script in a browser; fixed by escaping output and a content security policy), <b>cross-site request forgery</b> (a browser sending a user's credentials to your site from someone else's page; fixed with anti-forgery tokens and same-site cookies), and <b>credential stuffing</b> (trying passwords leaked from other breaches against your login; fixed with rate limiting on login attempts, multi-factor authentication, and checking new credentials against known-breached password lists).</p>`,
        deeper: `<p>One idea ties all of this together, worth stating explicitly if asked to design a secure system from scratch: <b>least privilege</b> - every user, service and token gets the minimum access it needs to do its job, nothing more, so a single compromise reaches as little as possible (its <b>blast radius</b>; see <a href="#sre-practices/srep-f3">Incident response: roles, communication and runbooks</a>) - and <b>defence in depth</b> - no single control is trusted alone; network boundaries, authentication, authorization checks, input validation and monitoring each catch what the others miss. Naming both, and applying them to the specific system in front of you rather than reciting them, is what separates a checklist answer from a design one.</p>`,
        check: {
    ...
    learn: [
      {
        ...
        part: 'field',
        title: 'Key terms',
        body: `<p>Terms specific to these ten cases, on top of the words already covered in the system design fundamentals chapter. Where a term belongs to another chapter, this list names it in a few words and links to the section that teaches it, instead of repeating a shorter version here. Read it once, then use the words freely.</p>
<ul>
<li><b>Ticket server.</b> A database that only issues ids; see <a href="#sql-databases/sql-f9">Online migrations, soft delete, and ticket servers</a>.</li>
<li><b>Fan-out on write / fan-out on read.</b> The two feed models; see <a href="#realtime-and-feeds/rf-f4">Feed generation</a>.</li>
...
<li><b>Cold start.</b> Two senses, and they are unrelated. See <a href="#serving-and-scale/ss-f2">Autoscaling, and where it stops</a> for the serving sense: the delay before a freshly started replica can answer, spent pulling an image, starting a process, loading a model and filling a cache. In the recommendation case below it means the opposite kind of problem - a new user or a new item with no history to base a recommendation on.</li>
<li><b>Idempotency key.</b> A value that makes a retry safe; see <a href="#apis-and-communication/apc-f6">Retries, timeouts, idempotency keys, and backoff</a>.</li>
...
<li><b>TTFT / TPOT.</b> Time to first token, time per output token; see <a href="#llm-inference-engineering/lie-f9">Measuring, cost, and observability</a>.</li>
<li><b>Guardrail.</b> A safety check around a model; see <a href="#rag-and-agents/raa-f9">Guardrails and safety</a>. What a guardrail looks for includes PII, personally identifiable information; see <a href="#sre-practices/srep-f7">Data retention, privacy and deletion</a>.</li>
<li><b>Model router.</b> Picks a model per request; see <a href="#rag-and-agents/raa-f8">Model routing: picking a model per request</a>.</li>
<li><b>Presence.</b> Who is online right now; see <a href="#realtime-and-feeds/rf-f3">A realtime database or presence system in brief</a>.</li>
<li><b>WebSocket / long polling.</b> Two ways to push to a client; see <a href="#realtime-and-feeds/rf-f1">Polling, Server-Sent Events and WebSockets</a>.</li>
</ul>`,
...
<p><b>Hard parts.</b> First, making the check atomic and fast under real concurrency: a naive read-then-write has a race that lets two concurrent requests both slip through when only one should. Second, choosing the algorithm: a token bucket is simple and allows a clean burst, while a sliding window is more precise about the sustained rate but slightly more work to compute. Third, doing this cheaply at extreme scale - centralising every check in Redis adds a network hop. A bounded alternative is for the shared authority to lease each replica a slice of a key's token budget. Replicas spend only their local leases and renew in batches, so latency falls while aggregate overshoot remains bounded; independently giving every replica the full or a higher threshold would multiply the global allowance.</p>
<p><b>Trade-offs an interviewer will push on.</b> Exact versus approximate limiting, and how much bounded over-limit traffic is acceptable in exchange for speed. Where to key the limit - IP stops one machine but not a botnet spread across many, while a per-account key stops one customer regardless of how many machines they use. And what happens when the shared store is unreachable: the correct policy depends on the endpoint and threat model. A low-risk public read may fail open with a conservative local emergency quota, while login, expensive compute, or abuse-sensitive endpoints may fail closed or shed traffic. Unreachable shared state should not silently mean unlimited traffic.</p>
<p><b>Where machine learning fits.</b> Rarely in the limiter itself, which should stay simple and predictable. The natural fit sits above it: an anomaly-detection model that adjusts limits per caller based on behaviour - flagging bot-like patterns for a tighter limit - layered on top of, not instead of, the plain counting limiter.</p>`,
        check: {
          question: 'Why might a team choose a bounded fail-open policy for a low-risk public-read endpoint when its shared counter store becomes unreachable?',
          options: [
            'Because failing open is always more secure',
            'Because denying all legitimate traffic may be worse than briefly under-enforcing the quota, provided a conservative local fallback still bounds load',
            'Because Redis cannot fail',
            'Because failing open removes the need for a fallback design'
          ],
          answer: 1,
          explain: 'A bounded fail-open policy is an endpoint-specific availability trade-off: a conservative local emergency quota keeps a low-risk API usable while limiting overload. Abuse-sensitive, authentication, or costly endpoints may instead fail closed or shed traffic.'
        }
      },
      {
        id: 'sdc-2',
...
<p><b>High-level design.</b> A charge request carries a caller-supplied idempotency key. The API first inserts a unique charge-attempt row with state <code>pending</code>; a concurrent retry finds that same row instead of starting another operation. The worker calls the external processor with the same idempotency key. A confirmed result is then committed locally with balanced double-entry <b>ledger</b> rows - a debit and a credit that sum to zero - and the attempt becomes <code>succeeded</code> or <code>failed</code>. Retries return the stored terminal result or the current processing state. A separate nightly job pulls the processor's settlement report and reconciles it line by line against the internal ledger.</p>
<p><b>Hard parts.</b> A local database transaction cannot be atomic with a network call to an independent processor. The uniqueness constraint atomically elects one local attempt, while the processor's own idempotency key prevents repeated submissions from becoming repeated charges. If a processor call times out, the outcome is <code>unknown</code>, not failed: resolve it by looking up that key at the processor or waiting for its webhook before deciding whether to submit again or write the ledger. This coordinates two idempotent state machines rather than claiming one cross-system transaction. The ledger itself remains immutable and balanced so the full history is reconstructable and reconciliation has something concrete to check.</p>
    ...
    activities: [
      {
        ...
        timeboxSec: 900,
        rubric: `Marking guide, out of 10. (1) Picks token bucket or a sliding-window counter and justifies short bursts rather than using a fixed window alone. (2) Runs the limiter at the gateway before expensive work. (3) Identifies cross-replica concurrency and uses an atomic shared-store operation. (4) Meets the 5ms p99 budget with a colocated store or bounded token leases/quota slices to replicas; rejects giving every replica the full or a higher threshold because that multiplies the global allowance. (5) Keys by API key with a reason. (6) Chooses failure policy per endpoint and threat model: fail closed or shed load for abuse-sensitive/costly paths, or bounded fail open with a conservative local emergency quota for lower-risk reads. (7) Returns 429 and Retry-After. (8) Gives a rough store-capacity estimate. (9) Avoids unnecessary consensus for an approximate control. (10) Plain, precise language. Deduct for: unbounded fail open; independent full per-replica limits; no store-failure answer; IP-only keying. {{HONESTY}}`,
        model: `"Algorithm: a sliding-window counter, not a hard token bucket, because I want burst tolerance without the fixed-window boundary problem - a caller shouldn't get double the rate by timing two bursts across a minute boundary. Key: the API key, not the caller's IP, since the thing I'm actually protecting is fairness between API consumers, and many callers can share one IP behind NAT (network address translation, where a shared device gives several machines one public IP address).

Where it lives: at the gateway, before the request reaches any service logic, so a rejected call costs almost nothing.

Shared state: with 40 replicas checking one key, I need an atomic shared authority - for example Redis with one check-and-update command. I would colocate it with the gateways. If a network hop still misses the 5ms budget, the authority can lease bounded token slices to replicas and replenish them in batches. A replica spends only its lease, so the aggregate remains close to 100 requests per second with bounded overshoot. Giving all 40 replicas a full or higher local threshold would multiply the intended quota.

Failure mode: I would classify endpoints. Login, abuse-sensitive, or expensive work fails closed or sheds load. A low-risk public read can fail open only against a conservative local emergency quota derived from its last lease; it never becomes unlimited just because Redis is unavailable.
...
Response contract: a 429 with a Retry-After header telling well-behaved clients exactly when to try again."`
      },
      {
        ...
        timeboxSec: 900,
        rubric: `Marking guide, out of 10. (1) Requires a client idempotency key and creates one unique local attempt record before the external call. (2) Explicitly rejects the claim that a local database transaction or uniqueness constraint can make the external processor call atomic; it only elects one local attempt. (3) Sends the same idempotency key to a processor that supports it and models pending, succeeded, failed, and unknown outcomes. (4) Resolves an ambiguous timeout by processor lookup or webhook before another submission or ledger write. (5) Models money movement as balanced double-entry ledger rows committed in a real local transaction. (6) Returns a stored terminal result, or processing status, on retry. (7) Reconciles the processor settlement report against the ledger and holds mismatches for investigation. (8) Mentions sharding by account rather than relaxing consistency. (9) Keeps the correctness path deterministic rather than ML-based. (10) Plain, precise language. Deduct for: claiming a local transaction spans the processor; updating a balance in place; treating timeout as failure; blind processor retries; silently correcting mismatches. {{HONESTY}}`,
        model: `"Request flow: the client sends a caller-generated idempotency key. I first insert one unique charge-attempt row in a short local transaction with state pending. A concurrent retry hits the uniqueness constraint and reads that attempt; it cannot create a second local operation. The database transaction does not include the processor call, so I would not claim the constraint alone prevents an external double charge.

The worker sends that same key to a processor that provides idempotent charge creation. If the call succeeds or definitively fails, I commit the terminal result locally. If it times out, the attempt becomes unknown rather than failed: I query the processor by key or wait for its webhook before another submission or ledger write. A client retry receives the stored result or a processing response while that resolution continues.
...
Scaling: as volume grows, I'd shard the ledger by account rather than loosen consistency, since account-level operations rarely need to join across shards."`
      },
      {
        ...
        timeboxSec: 120,
        rubric: `Must-haves: (1) explains that retrieval can return a topically related chunk that does not support the claim, while generation can misuse even the right chunk; retrieval supplies evidence but does not enforce grounding. (2) distinguishes retrieval failure from generation failure. (3) proposes better chunking/re-ranking for retrieval and a claim-to-source verification pass for generation. (4) says an unsupported claim must be removed, qualified, regenerated from supported evidence, or refused - never merely left in place after dropping its citation. (5) acknowledges residual risk, shows sources, and monitors by sampling answers and auditing citations. Common mistakes: blaming "the model" vaguely; treating RAG as a guarantee; proposing a bigger model as the whole fix; dropping only the citation; no monitoring. {{HONESTY}}`,
...
If it's generation: the model attached a citation without the claim actually being grounded in it. I'd add an explicit verification step that checks each material claim against its cited chunk before the answer is shown. If support is missing, I would remove or qualify the claim, regenerate from the supported evidence, or refuse that part of the answer. I would not merely drop the citation and leave the unsupported statement behind.
