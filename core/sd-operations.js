...
The trickier case is a call to the card processor that times out on our side but actually succeeded at the processor. I wouldn't blindly retry the charge - I'd first check the processor's own status for that idempotency key, since most processors accept the same key and will report the outcome of the original attempt, or wait for a webhook from the processor confirming the result, and only charge again if the processor genuinely has no record of it.
