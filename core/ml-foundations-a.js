...
(function (root) {
  ...
  root.PREP_CORE['ml-math-essentials'] = {
    ...
    level: 'danger',
    levelLabel: 'Commonly asked in machine learning interviews, in some form.',
    why: `Many trained models, especially differentiable ones, use linear algebra to represent data and parameters, calculus to compute gradients, probability to reason about uncertainty, information theory to measure prediction error, and an optimiser to search for parameters that reduce a loss. This chapter brings together mathematical tools that often come up in machine learning interviews and helps connect model behavior to the operations that produce it. Working through the numbers by hand can make the formulas easier to use.`,
    readings: [
    ...
    learn: [
      {
        ...
        part: 'field',
        title: 'Key terms',
        body: `<p>These terms follow the chapter's dependency order: data representations first, then calculus and probability, followed by prediction losses and optimisation.</p>
<ul>
...
<li><b>Eigenvector, eigenvalue</b> -- a nonzero vector that a matrix maps to a scalar multiple of itself, and that scalar. For a covariance matrix, the eigenvectors give PCA directions and the nonnegative eigenvalues give the variance along them.</li>
...
<li><b>Entropy, cross-entropy, KL divergence</b> (KL is short for Kullback-Leibler, the two people it is named after) -- how much uncertainty a distribution has; the expected code cost when outcomes from the true distribution are encoded using a guessed distribution; and the extra cost of that guess over entropy. The cost units depend on the logarithm base.</li>
...
<li><b>Convex function</b> -- a function whose line segment between any two graph points lies on or above the graph; any local minimum is global, though there may be several minimizers.</li>
<li><b>Local minimum, saddle point</b> -- a point that looks like the bottom from nearby but is not the true minimum; and a point that is a minimum in some directions and a maximum in others.</li>
        ...
        deeper: `<p>These terms are useful in interviews about how a model works. Being able to explain what a gradient is, which direction a parameter moves, and why connects the name of the mechanism to its behavior. One way to check your understanding is to pick a term above and work through its example from memory before reading on.</p>`,
        check: {
...
<p><b>Backpropagation is the chain rule applied through a graph.</b> A neural network is a chain of simple functions -- each layer feeds the next. The <b>forward pass</b> computes and stores every intermediate value, layer by layer, ending in the loss. The <b>backward pass</b> starts at the loss with a gradient of 1, then walks back through the graph one layer at a time, at each step multiplying the gradient arriving from the layer above by that layer's own local derivative -- exactly the chain rule, applied once per layer instead of once per parameter. This is what makes it efficient: one backward pass computes the gradient with respect to every parameter in the network at once, by reusing the same intermediate values the forward pass already stored.</p>`,
        deeper: `<p>Backpropagation is more than a statement that the chain rule applies: its practical value comes from reusing intermediate values and gradients. Differentiating a million-parameter network separately for each parameter would require a great deal of repeated work. Backpropagation computes all parameter gradients in one forward pass and one backward pass by sharing intermediate calculations across the graph.</p>`,
        check: {
...
<p><b>Convexity.</b> A function is convex if the line segment between any two points on its graph lies on or above the graph. Any local minimum of a convex function is global, though a minimum need not be unique or even attained. A convergence guarantee for gradient descent needs more assumptions: for example, if the objective is differentiable and has an L-Lipschitz gradient, where L bounds how quickly the gradient can change, a global minimizer exists, and the fixed step size satisfies 0 &lt; η ≤ 1/L, gradient descent's objective values converge to the global minimum value. Most deep-network losses are non-convex; gradient-based methods often find solutions that generalise well in practice, though this behavior is not fully explained theoretically.</p>`,
...
<p>One thing worth saying out loud in an interview: this trick is why every production softmax implementation, including the one inside PyTorch's cross-entropy loss, does the max-subtraction internally rather than trusting the caller to have well-scaled logits.</p>`
    ...
    glossary: {
      ...
      rows: [
        ...
        ['Convexity', 'A function whose line segment between any two graph points lies on or above the graph; every local minimum is global.', 'Convex optimisation objectives; many deep-network losses are non-convex.', '']
      ]
    }
  };
  ...
  root.PREP_CORE['data-and-generalization'] = {
    ...
    level: 'danger',
    levelLabel: 'Commonly asked in machine learning interviews, often as "how do you know this model will actually work."',
    why: `A model learns from the data it receives, and its performance on that sample may not carry over to new data. Many production problems and irreproducible offline results involve ideas in this chapter, such as leakage, sampling bias, or a change between training and live data. Interviewers often ask about these issues because they test practical judgment alongside model knowledge.`,
    readings: [
      ...
      { l: 'Kaggle Learn -- Data Leakage', u: 'https://www.kaggle.com/code/alexisbcook/data-leakage/tutorial', w: 'Short, concrete, worked examples of target leakage and train-test leakage with real (if simplified) tabular data, which is exactly the intuition an interviewer is testing for.', m: 15 },
    ...
    learn: [
      {
        ...
        part: 'field',
        title: 'Key terms',
        body: `<p>These terms follow the chapter's dependency order: how data is sampled, how inputs are represented, how training fit differs from new-data performance, and how that relationship can fail.</p>
<ul>
...
<p>A practical habit is to add a binary "was this missing" indicator alongside an imputed feature. The indicator may help a model use missingness as information, but its relationship with the target does not identify why values are missing.</p>`,
        deeper: `<p>The indicator lets a tree-based model learn different rules for missing-and-imputed rows and observed rows that share the same imputed value. It can also show that missingness is associated with the target. That association alone cannot distinguish MCAR, MAR, and MNAR: under MAR, missingness can be associated with the target through observed variables, and an absent association does not prove MCAR. Identifying the missingness mechanism requires assumptions or additional information.</p>`,
        check: {
...
<p><b>Target encoding</b> replaces a category with the average target value for that category. Worked example: if customers in category "Region: East" have a historical churn rate of 0.32, every "East" row gets the numeric value 0.32 in place of the category label. This can be a powerful encoding for high-cardinality categorical features, but it carries a specific <b>leakage risk</b>: computing the average target value using the very rows you are about to train on lets each row's own label leak into its own feature. The standard fix is to compute the encoding using only out-of-fold data (a different fold's rows, in cross-validation) or a held-out portion of the training set, often combined with smoothing toward the overall average for categories with few observations: <code>smoothed_mean = (n_c * mean_c + m * global_mean) / (n_c + m)</code>, where n_c is the count of rows in that category and m is a smoothing weight.</p>`,
      ...
      },
      {
        ...
        body: `<p>Data leakage means information reaches training that would not actually be available when the model makes a real prediction; it is one common reason a model looks excellent offline and performs badly in production. Let <code>t0</code> mark the prediction or decision time, and let <code>H</code> be the future outcome window, so <code>t0+H</code> is when that label becomes known. Three recurring forms.</p>
...
<p><b>Temporal leakage.</b> A feature that was not actually knowable at the moment the prediction would have been made in production. A classic real example: a churn model with a feature "number of support tickets in the customer's final month" -- customers who are about to churn often contact support a lot right before leaving, so this feature is strongly predictive in a backtest, but at real prediction time (say, 30 days before a hypothetical churn date) that final month has not happened yet. Fix: for every row, enforce that every feature's value was computed using only information available strictly before that row's prediction timestamp, sometimes called <a href="#data-features/df-v2-time-leakage">point-in-time correctness</a>.</p>
<p><b>Target leakage.</b> A feature that is, in effect, a proxy for the label, often because it is a downstream consequence of the same event the label represents rather than a genuine predictor of it. For example, in a hypothetical hospital readmission model, a feature recorded only after discharge or readmission could partly encode the outcome rather than predict it. Fix: for every candidate feature, ask explicitly "would this value exist, in this form, at prediction time?" If the answer is no, exclude it.</p>`,
...
<p><b>The PAC (probably approximately correct) idea</b>, in plain words: given enough independent, identically distributed training examples, a model that fits the training data well will, with high probability, also perform well on new data -- and the framework makes both "well" and "high probability" precise, quantitative promises rather than hopes. A simplified version of the classic bound, for a finite set of candidate models H (the "hypothesis class"), states that <code>n ≥ (1/ε) * (ln|H| + ln(1/δ))</code> training examples are enough to guarantee, with probability at least 1-δ, a model that fits the training data perfectly will have true error at most ε. Worked example: with 1,000 candidate models (|H| = 1,000), wanting error at most ε = 0.1 with confidence 1-δ = 0.95 (so δ = 0.05): <code>n ≥ (1/0.1) * (ln(1000) + ln(20)) = 10 * (6.908 + 2.996) ≈ 99</code> -- about 100 examples are enough, in this idealised setting.</p>`,
      ...
      },
      {
        ...
        body: `<p>A model trained on data from one point in time is being deployed against a future you cannot observe in advance, and the relationship between features and labels can move. In the notation below, <code>P(X)</code> is the input-feature distribution, <code>P(Y)</code> is the label distribution, <code>P(Y|X)</code> is the label distribution given the inputs, and <code>P(X|Y)</code> is the input distribution given a label. Three distinct kinds of shift are useful to tell apart because they suggest different checks.</p>
...
<p><b>Detection.</b> Covariate shift can be checked by comparing input-feature distributions between training and live data. A common comparison metric is the <b>population stability index (PSI)</b>: <code>PSI = sum over bins of (actual_pct - expected_pct) * ln(actual_pct / expected_pct)</code>. A PSI above roughly 0.25 is a commonly used heuristic for a potentially important shift, not a universal threshold; interpretation depends on binning, sample size, baseline, and the costs of acting or waiting. Label shift needs fresh labels, even a small delayed sample, to compare P(Y) over time. Concept drift is the hardest: since input features alone do not reveal it, it usually needs monitoring accuracy or <a href="#classical-models/cm-f3">calibration</a> against fresh ground truth, or a proxy signal that correlates with the true label and arrives sooner. What to do after a shift is detected -- alert, retrain, or roll back -- is <a href="#reliability-ops/ro-f3">covered in Reliability and operations</a>.</p>`,
        deeper: `<p>Worked PSI example, with two bins: training data had 60% of customers in a "young" age bin and 40% in "old"; live data now shows 40% "young" and 60% "old." <code>PSI = (0.40 - 0.60)*ln(0.40/0.60) + (0.60 - 0.40)*ln(0.60/0.40) = (-0.20)*(-0.405) + (0.20)*(0.405) ≈ 0.081 + 0.081 = 0.162</code>. The value is below a commonly used 0.25 heuristic, but that alone does not show the shift is harmless or rule out action. Interpret PSI with its binning, sample size, baseline, and the operational cost of missing a change; a monitoring threshold should be chosen for the model and its use.</p>`,
        check: {
...
<p>Finally, for evaluation, I would split by time -- train on earlier months, validate on a later month -- rather than a random shuffle, because a random split lets future months leak backward into training and does not test what the model will actually face: predicting forward in time it has never seen.</p>`
      },
      {
        ...
        timeboxSec: 180,
        rubric: 'Must-haves: (1) situation 1 is closer to a covariate shift or a broken assumption about the serving context (the input distribution of "where and how the item is shown" changed) rather than concept drift on the item-level relevance relationship itself, and is detectable by monitoring input/context feature distributions; (2) situation 2 identifies a training-coverage gap that creates a mismatch between the training sample and the holiday deployment period, even though the seasonal process is recurring rather than newly changed. Recommend adding representative historical holiday data or seasonal features; distinguish this known coverage gap from newly emerging drift; (3) situation 3 is concept drift -- the same price and product now produce a different customer response because a new competitor changed the relationship P(response|price), and detecting it needs a monitored business metric (actual conversion versus predicted) against fresh outcomes, since the price feature itself has not moved. Bonus: names PSI or a similar distributional distance for the covariate case. Common mistakes: calling every situation concept drift without distinguishing them; treating a known gap in seasonal training coverage as proof that the underlying process newly changed. {{HONESTY}}',
...
<p>Situation 2: this is a known seasonal-coverage gap that creates a train/live mismatch: holiday-period examples are absent from training but present at deployment. Because the season recurs, this is not evidence that the underlying process newly changed; it is a predictable gap in the training sample. I would add representative historical holiday data and useful seasonal features, then validate on a later holiday period. Monitoring can still detect the difference between training and live inputs, but it does not replace collecting the missing coverage.</p>
