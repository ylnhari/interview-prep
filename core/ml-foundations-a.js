...
<li><b>PAC learning</b> -- a formal framework guaranteeing that, given enough i.i.d. samples, a model that fits the training data well will, with high probability, also generalise well.</li>
...
<p>A practical habit that catches MNAR before it causes damage: add a binary "was this missing" indicator column alongside any imputed feature. If that indicator itself turns out to be predictive of the target, missingness is carrying information, and a naive fill-in-and-forget imputation was about to throw that information away.</p>`,
      ...
      },
      {
        ...
        body: `<p><b>Scaling.</b> Standardisation rescales a feature to mean 0 and standard deviation 1: <code>z = (x - mean) / standard_deviation</code>. Worked example: a feature with mean 50 and standard deviation 10 -- a raw value of 65 standardises to <code>(65 - 50) / 10 = 1.5</code>, meaning 1.5 standard deviations above average. <b>Min-max scaling</b> instead maps a feature into a fixed range, usually 0 to 1: <code>x' = (x - min) / (max - min)</code>. Standardisation is the default for algorithms sensitive to feature scale, such as anything using a distance calculation, gradient descent on unscaled inputs, or L1/L2 regularisation (an unscaled feature with large raw values gets penalised differently from one with small raw values purely because of units, not because it matters more). Min-max scaling is preferred when a bounded range is required by the model itself, such as some neural network input layers, or when the interpretation "0 to 1" is meaningful to a downstream consumer.</p>
...
<p><b>Temporal leakage.</b> A feature that was not actually knowable at the moment the prediction would have been made in production. A classic real example: a churn model with a feature "number of support tickets in the customer's final month" -- customers who are about to churn often contact support a lot right before leaving, so this feature is strongly predictive in a backtest, but at real prediction time (say, 30 days before a hypothetical churn date) that final month has not happened yet. Fix: for every row, enforce that every feature's value was computed using only information available strictly before that row's prediction timestamp, sometimes called point-in-time correctness.</p>
...
<p><b>Detection.</b> Covariate shift is detectable by comparing the distribution of each input feature between training and live data -- a common metric is the <b>population stability index (PSI)</b>: <code>PSI = sum over bins of (actual_pct - expected_pct) * ln(actual_pct / expected_pct)</code>, with a PSI above roughly 0.25 usually read as a significant shift. Label shift needs a source of fresh labels, even a small delayed sample, to compare P(Y) over time. Concept drift is the hardest: since the inputs alone do not reveal it, it usually needs monitoring an actual accuracy or calibration metric against fresh ground truth, or a proxy signal that correlates with the true label but arrives sooner.</p>`,
