...
(function (root) {
  ...
  root.PREP_CORE['rag-and-agents'] = {
    id: 'rag-and-agents',
    title: 'Retrieval-augmented generation (RAG) and agent workflows',
    level: 'danger',
    levelLabel: 'Core focus: design current, evidence-based, access-controlled LLM features.',
    why: `Learn to design a retrieval-augmented generation system that keeps evidence current and authorized, separates retrieval problems from generation problems, and measures quality, latency, and cost. You will also learn when a fixed workflow is enough and when tools or an agent earn their extra complexity.`,
    learn: [
      {
        id: 'raa-0',
        part: 'field',
...
<li><b>Embedding</b> - <a href="#deep-learning-essentials/dle-f9">taught in Deep learning essentials</a>: a list of numbers (a vector) produced by a trained model, representing the meaning of a piece of text.</li>
...
<li><b>Personally identifiable information (PII)</b> - information that can identify a person, such as a full name combined with contact or government-identification details; handling rules decide whether it may be sent to a model or written to a log.</li>
...
<li><b>Data leakage in a retrieval system</b> - private, internal, or another customer's information appearing somewhere it should not, such as in a model's answer. A different thing from the <a href="#data-and-generalization/dag-f5">training-data leakage</a> of the foundations chapters.</li>
<li><b>Model router</b> - <a href="#rag-and-agents/raa-f8">explained below</a>: a component that picks which model answers a given request, to balance cost, speed, and quality.</li>
</ul>`,
...
<p><b>First, write a clearer prompt.</b> A surprising share of "the model does not know X" problems are actually "the instructions were not clear enough" problems, and a better system prompt has no training run and can be changed in seconds. Its tokens still have a per-request cost.</p>
<p><b>Second, put the information straight into the prompt.</b> If the material the model needs is small and known in advance, paste it directly into the prompt instead of building any retrieval system at all. This is the simplest possible design. It does not scale: a longer prompt costs more, runs slower, and resends the same tokens on every request, though <a href="#rag-and-agents/raa-f6">prompt caching</a> softens that cost when the same block of text repeats across requests.</p>
...
<p><b>Fine-tuning can encode knowledge, but it is usually a poor default for fast-changing, source-traceable facts.</b> It changes behaviour - format, tone, a narrow classification scheme, following a house style reliably - and each fact update requires retraining and re-evaluation. Retrieval can instead supply current evidence and show its source at request time.</p>
<p>A production system commonly uses more than one of these at once: retrieval for facts, a fine-tune for the output format that wraps around them, and a clear prompt tying the two together.</p>`,
      ...
      },
      {
        id: 'raa-f1a',
        part: 'field',
        title: 'A production RAG system has an offline path and an online path',
        viz: 'rag-two-paths',
        body: `<p>A production retrieval-augmented generation (RAG) system does two different jobs. The <b>offline ingestion and indexing path</b> runs when a source changes: it turns documents into a current, governed index. The <b>online query path</b> runs for each question: it decides what the requester may see, finds evidence, builds a bounded context, and returns an answer. The output of the first path is not just vectors; it is a versioned set of chunks with provenance and access metadata. The output of the second is an answer with the evidence and checks used to produce it.</p>
<p>Keeping the paths separate prevents a common mistake: treating a search index as a static bag of text. A current policy, an old policy, a deleted document, and a document whose permissions changed are different retrieval states. The index must preserve a source identifier, current version or effective date, useful metadata, and the access-control list (ACL) or other policy that governs each chunk. A source deletion or permission revocation must propagate to every retrievable representation and protected cache before a later request can serve it.</p>
<p><b>Component responsibilities.</b></p>
<ul>
<li><b>Source and ingestion.</b> Read documents, parse text or use optical character recognition (OCR) for scans, and preserve tables or code structures that plain text would scramble. Keep the source version, parser version, parent document and access metadata with the extracted content. <a href="#rag-and-agents/raa-f2">Chunking</a> decides how that content becomes independently retrievable evidence.</li>
<li><b>Index publication.</b> Build a lexical index, vector embeddings and a vector index, or both. Embeddings are needed for vector search, not for a lexical-only design. A product choice is not an access-control decision. Validate representative extraction and indexing failures, then publish a tested index version. Incremental updates can avoid rebuilding unchanged documents, but an old chunk must not survive a deletion, replacement, or permission change.</li>
<li><b>Request authorization and retrieval.</b> Authenticate the requester, derive their permitted scope on the server, and apply it before lexical search, vector search, parent expansion, reranking, or prompt construction. A filter after generation is too late: unauthorized text has already entered the model context or its logs. A protected cache key must also include every audience and version dimension that changes the authorized result, rather than only the natural-language question.</li>
<li><b>Context and generation.</b> A query may be rewritten when evaluation shows it helps. Retrieve a broad candidate set, optionally rerank it, then select only the evidence that fits an explicit prompt-token budget. Mark retrieved text as untrusted evidence, not instructions. The model should be able to abstain or escalate when evidence is missing, conflicting, or insufficient.</li>
<li><b>Answer checks.</b> Return source citations beside claims and validate the required output format. A citation is a pointer, not proof: a check must verify that the cited passage supports the claim. Deterministic checks are useful where possible; a model-based judge is a measured classifier with false accepts and false rejects, not a guarantee.</li>
</ul>
<p><b>Measure the paths separately.</b> For ingestion, measure coverage, freshness, deletion propagation, parser failures, and access-policy correctness on representative scans, tables, lists, and code. For retrieval, use the versioned golden set from <a href="#rag-and-agents/raa-f5">RAG evaluation</a> to measure whether permitted evidence was found and ranked well. For generation, measure answer correctness, faithfulness, citation support, and appropriate abstention separately. Trace the source and index version, permission decision, selected evidence, prompt/model configuration, latency, and cost; content logging still follows classification, access, and retention policy.</p>
<p>There are also two capacity budgets. Offline work consumes extraction, embedding, indexing, storage, and update throughput. Online work consumes retrieval and reranking latency, prompt and output tokens, model-serving capacity, and cost per successful task. Measure them on the real corpus and request mix rather than assuming a universal chunk size, store, model, or validator. Retries need their own bounded attempts, deadline, token, and cost budget; retry a transient service failure differently from a missing permission or insufficient evidence.</p>
<p>This architecture is RAG, not automatically an agent. A fixed retrieve-and-answer workflow is usually simpler when the steps are known. Add query decomposition, tools, iteration, or an agent only when evaluation shows that a query class needs them, then apply the stopping, permission, and observability controls in <a href="#rag-and-agents/raa-v2-safe-agent-stopping">Agents: tools, planning, and stopping rules</a>.</p>`,
        deeper: `<p>The diagram names a baseline architecture, not a required vendor stack or a promise that retrieved context makes every answer correct. Start with the smallest path that meets measured freshness, quality, security, latency, and cost targets. Add structure-aware parsing, hybrid retrieval, reranking, query rewriting, validators, or agentic steps one at a time and keep the evidence showing why each addition earned its complexity.</p>`,
        check: {
          question: 'Where must document authorization be enforced in a RAG system that serves private material?',
          options: [
            'Only after the model writes an answer, by removing forbidden words',
            'Before retrieval and context construction, across every search, expansion, reranking, and protected cache path',
            'Only when a document is first embedded, because its access rules cannot change',
            'Inside the model prompt, because the model can be instructed not to disclose private text'
          ],
          answer: 1,
          explain: 'Post-answer filtering cannot undo text already supplied to the model. Permissions can change after indexing, so the request path must enforce the current permitted scope before any candidate becomes context or a cache hit.'
        }
      },
      {
        id: 'raa-f2',
...
<p><b>What breaks with PDFs</b> (PDF is short for portable document format) <b>and tables.</b> A PDF can contain tagged structure, but many files mainly encode positioned text rather than a reliable reading order: a multi-column layout can be extracted left-to-right across two unrelated columns, headers and footers can be interleaved into the body text, and a scanned page has no extractable text without OCR (optical character recognition, which reads text from an image). A table loses its row-and-column meaning when it is flattened into plain text, since "the value in this column, this row" becomes just another sequence of words with the structure gone; represent it in a row/column-aware form, keep a suitable table unit together, or test another approach against real questions.</p>`,
        deeper: `<p>Chunk size is a trade-off with retrieval precision, not a fixed setting: a smaller chunk lets a search match a narrow, specific question more precisely, while a larger chunk carries more surrounding context that a smaller chunk might have cut away. One design can retrieve a small chunk for matching accuracy, then expand it to immediate neighbours before prompt construction; evaluate that choice on the corpus and context budget.</p>`,
        check: {
...
<p><b>Sparse retrieval</b> compares exact words, most commonly scored with <b>BM25</b> (best matching 25), the ranking formula <a href="#search-and-retrieval/sr-f2">derived in Search and retrieval systems</a>. It reliably finds an exact term, including ones a dense model was never trained on, but it cannot match a paraphrase with no shared words.</p>
<p><b>Hybrid retrieval</b> runs both searches and combines their two rankings into one. One option is <b>reciprocal rank fusion</b>: each document gets a score based on where it ranked in each list, and documents that rank well in either list rise in the combined result. Hybrid retrieval can cover both meaning-based and exact-word matches; compare it with sparse-only or dense-only retrieval on the actual corpus, queries, filters, latency, and cost.</p>
<p><b>Reranking</b> takes a first search's shortlist and scores each query-chunk pair again with a cross-encoder model that reads the query and chunk together rather than comparing separate embeddings. This costs more than first-pass search, so it is evaluated only on a shortlist. It can improve ranking quality, but the quality gain and latency cost need to be measured on a labelled set.</p>
<p><b>Query rewriting</b> changes the user's question before any of this runs: expanding an abbreviation, splitting a multi-part question into several searches, or rephrasing a vague question into one closer to the wording used in the source documents.</p>`,
        deeper: `<p>One evaluated pipeline can rewrite a query first, use hybrid search for a wide candidate set, and rerank before selecting the few chunks that enter the prompt. Each stage adds latency, cost, and possible failure modes, so retain it only when its measured benefit justifies it.</p>`,
        check: {
...
<p><b>IVF</b> (inverted file index) takes a different approach: it first clusters the vectors, typically with <a href="#classical-models/cm-f9">k-means</a>, into a fixed number of groups, each represented by a centroid. At query time, it compares the query only against the centroids nearest to it, then searches only inside those chosen groups, called probes, rather than the whole index.</p>
<p><b>The recall versus latency trade-off.</b> Every ANN index has a knob that trades one for the other: HNSW's <b>ef</b> parameter controls how many candidates it keeps at each step of the search, and IVF's <b>nprobe</b> controls how many clusters it searches. Turning either one up finds more of the true nearest neighbours (higher recall) at the cost of checking more of the index (higher latency); turning it down speeds up the search but raises the chance of missing a neighbour that a full search would have found.</p>`,
        deeper: `<p>HNSW is a common graph-based ANN option whose graph connections use memory. IVF partitions vectors into clusters and can have different memory, build-time, recall, and latency behaviour. The right comparison depends on implementation, index parameters, dataset, filter selectivity, build constraints, and the target recall; benchmark the chosen options on the workload rather than treating either as a default.</p>`,
        check: {
...
<p><b>Retrieval recall</b> asks whether known relevant evidence was found: out of the questions whose evaluation set identifies one or more relevant passages in the index, what fraction of the time does retrieval return enough of that evidence in its top results (commonly measured as recall@k, for some small k such as 5 or 10). If recall is low, prompting cannot make omitted evidence available for a grounded answer. A model may still answer from its parameters, so measure answer correctness separately from support by retrieved evidence.</p>
<p><b>Answer faithfulness</b> (also groundedness) asks a separate question: given that the model received sufficient relevant evidence, does its answer stick to what that evidence says, or does it add unsupported claims. A model can receive relevant evidence and still hallucinate on top of it.</p>
<p><a href="#llm-fine-tuning-and-alignment/lft-f5">LLM-as-judge</a> is the common way to score faithfulness at scale: the judge is given the answer and the retrieved chunks, and asked whether each claim in the answer is supported by those chunks. The fine-tuning chapter covers the biases a judge carries and how to check it; what is specific to a faithfulness judge is the prompt it gets. Ask it to work claim by claim rather than to rate the whole answer, require it to quote the supporting sentence for each claim it passes, and count an unquotable claim as unsupported. A judge asked only "is this answer grounded?" tends to agree with any fluent answer.</p>
<p><b>A golden set</b> is a small, carefully built set of real questions with a known correct answer when one is supported and a known relevant source passage or evidence set for each one. It turns "the demo looked good" into numbers you can track release over release, including questions where abstention is the right result.</p>`,
...
<p><b>The context window</b> is the hard limit on how many tokens a request can contain, prompt plus generated answer combined. As a conversation grows, earlier turns take up more of that budget. The application must then deliberately summarize or truncate selected history, or the provider may reject an over-limit request; a sliding-window application often drops the oldest content, but that is an application policy rather than a universal model behavior.</p>
...
<p><b>Observability and tracing</b> records each decision and tool call - tool, authorized operation, status, timing, identifiers, versions, and policy result - so a bad outcome can be traced back to the specific decision that caused it instead of only being visible as a wrong final answer. Minimize or redact raw arguments and results when classification, access, or retention policy does not authorize them; hashes and references can often supply the link without copying protected content.</p>`,
        ...
        check: {
          ...
          options: [
            ...
            'A stopping rule, such as a maximum number of steps, that ends the loop with a supported answer, abstention, or escalation',
            'Giving the agent more tools to choose from',
            'Removing the retry logic entirely'
          ],
          answer: 1,
          explain: 'Without an explicit stopping rule, nothing tells the agent to stop. A maximum step count or a cost budget must end the loop in a safe outcome: a supported answer when evidence is sufficient, otherwise abstention or escalation.'
        }
      },
      {
        id: 'raa-f8',
...
<p><b>Input checks</b> run on what is about to be sent to the model: detecting personally identifiable information (PII) so it can be redacted or blocked, scanning for known prompt-injection patterns, and filtering content that violates a stated policy before it ever reaches the model.</p>
...
<p><b>Data leakage</b> in this setting covers a system prompt or internal instructions appearing in a model's answer, one customer's retrieved documents appearing in another customer's answer when many customers share one deployment, and a model's training data resurfacing content it should not repeat. Each needs a different guard: strict separation of what gets retrieved per customer; keeping secrets and authorization policy outside model-visible system text, with data and tool permissions enforced by the surrounding system; and output checks that catch leaks as defense in depth. Instructions and output checks are not access control on their own.</p>`,
        deeper: `<p>Guardrails work in layers because no single check catches everything. Input filters and output checks may reduce or detect some known attack and leak patterns, but neither guarantees coverage. Treat retrieved text as untrusted, and enforce the hard boundary with authorization before retrieval, least-privilege tools, structured validation, and confirmations for consequential actions. Relying on input checks alone leaves indirect injection largely unaddressed.</p>`,
        check: {
...
<p>For evaluation, I'd build a golden set of maybe 50 to 100 real policy questions with known correct answers and known source sections, and track retrieval recall and answer faithfulness separately, since a low recall number sends me back to chunking or retrieval, while a low faithfulness number with good recall sends me to the prompt or a reranker instead. I have not built this exact pipeline in production; this is the design I would follow, not a report of having shipped it."</p>`
      },
      {
        id: 'raa-a4',
        type: 'design',
        title: 'Exercise: draw a production RAG system for controlled documents',
        viz: 'rag-two-paths',
        prompt: 'Design a knowledge assistant over weekly-changing technical documents. The corpus includes scanned pages, tables, deleted or superseded policies, and documents that different teams may or may not read. Draw the offline ingestion/indexing path and the online query path. For each, state its output, how current access rules are enforced, how you would measure it, and which capacity or cost budget matters. Explain what happens when the evidence is missing.',
        timeboxSec: 540,
        rubric: `Must-haves: (1) separates an offline ingestion/indexing path from an online query path and gives each a distinct output; (2) covers parsing/OCR/table handling, chunking, source/version metadata, embedding/indexing, validation, and publication of an updated index; (3) says that superseded, deleted, or permission-changed source material must stop being retrievable, including through protected caches; (4) authenticates the requester and enforces current authorization before every retrieval, expansion, reranking, context, and cache path, not with an after-answer filter; (5) retrieves permitted candidates, uses hybrid search or gives a reasoned alternative, treats reranking as optional, and keeps selected evidence within a stated context budget; (6) treats retrieved text as untrusted, supports citations with an actual claim-support check, and defines an abstain or escalation outcome for insufficient evidence; (7) separates ingestion, retrieval, generation, and end-to-end measurement, including freshness/deletion propagation, retrieval quality, faithfulness/citation support, safety, tail latency, and cost; (8) names separate offline and online capacity budgets and bounds retries by attempts, time, tokens, and cost; (9) does not claim that a named vendor, multi-agent design, or extra model validator is required. Common mistakes: using one static index with no source-version or deletion path; applying ACL filtering after the model has seen the text; assuming citations or a second model prove correctness; treating a cache as safe when keyed only by the query; adding an unbounded retry or agent loop. {{HONESTY}}`,
        model: `<p>"I would draw two paths. Offline, a source-change worker parses documents, uses OCR when needed, preserves table structure, and records source ID, version, parent document, and access metadata before chunking. It builds evaluated lexical, vector, or hybrid indexes. Its output is a tested index whose chunks still point to their source. A replacement, deletion, or permission change invalidates old chunks and protected cache entries before that version can serve a query.</p>
<p>Online, the application authenticates the requester and derives permitted document scope on the server. Every candidate search, parent expansion, reranker, and cache lookup is constrained by that scope before text reaches the prompt. I would retrieve permitted candidates, use a reranker only if its measured quality gain pays for its latency, and place only the best evidence inside a context budget. Retrieved documents are evidence, not instructions. The answer cites evidence, and I would check each important claim against it and return an explicit "I do not have enough permitted evidence" outcome when it does not.</p>
<p>I would measure offline coverage, freshness, parser failures, deletion propagation, and access-policy tests. Online, I would measure golden-set retrieval quality, answer correctness, faithfulness, citation support, abstention, safety, tail latency, and cost. The offline budget is extraction, embedding, indexing, and storage; the online budget is retrieval/reranking time, tokens, serving capacity, and cost per task. I would bound retries and iteration by attempts, deadline, tokens, and cost. Start with this fixed workflow; add an agent only when evaluation shows model-chosen steps improve the result."</p>`
      },
      {
        id: 'raa-a2',
        type: 'formulate',
...
<p>None of this makes the agent perfectly safe - a good enough injection might still shift its wording or its summary. But by putting the hard control on the actual risky action, sending mail to an arbitrary address, rather than trying to perfectly filter the model's reasoning, the worst outcome, data going somewhere the customer never approved, is the one thing I can actually rule out rather than just make less likely."</p>`
    ...
    sayItOutLoud: null,
    readings: [
      { l: 'Architecture walkthrough (sponsored video)', u: 'https://www.youtube.com/watch?v=4KiiKQ9RVvA', w: 'A secondary architecture walkthrough. Treat its vendor and multi-agent choices as optional design options, then check the primary sources and the chapter evaluation sections before adopting them.', m: 16 },
      { l: 'Google Research: Sufficient Context', u: 'https://research.google/pubs/sufficient-context-a-new-lens-on-retrieval-augmented-generation-systems/', w: 'Why relevant-looking context can still be insufficient, and why abstention and evidence use must be measured rather than assumed.', m: 15 },
      { l: 'Microsoft: Document-level access for search', u: 'https://learn.microsoft.com/en-us/azure/search/search-document-level-access-overview', w: 'The query-time authorization problem: document access must constrain retrieval before content becomes model context.', m: 15 },
    ...
    glossary: [
      {
        ...
        rows: [
          ...
          ['Hybrid retrieval', 'Running dense and sparse search together and combining the two rankings, for example by reciprocal rank fusion.', 'One option when a corpus needs both paraphrase and exact-identifier matching; compare it with single-method retrieval on measured quality, latency, and cost.'],
          ...
          ['HNSW', 'A layered-graph approximate nearest neighbour index, searched from a sparse top layer down to a dense bottom layer.', 'A graph-based ANN option to benchmark when recall, latency, and graph-memory trade-offs fit the workload.'],
          ['IVF', 'An approximate nearest neighbour index that clusters vectors first and searches only the nearest clusters.', 'A cluster-based ANN option to compare when dataset, filters, build constraints, and target recall favor it.'],
...
<li><b>Optimizer state</b> - the extra numbers an optimiser like Adam keeps per parameter, beyond the parameter and its gradient, to decide how to update it. Adam keeps two of them; in the common fp16 AMP layout with fp32 master weights, those states help make the fixed estimate 16 bytes per parameter.</li>
...
<li><b>Distributed data parallel (DDP)</b> - PyTorch's standard data-parallel implementation: each GPU holds a full replica, processes a batch shard, then synchronizes gradients.</li>
...
<li><b>Microbatch; F/B</b> - a small piece of a batch sent through a training pipeline; diagrams mark its forward pass as F and its backward pass as B.</li>
...
<p><b>A common 16-bytes-per-parameter estimate.</b> In the classic fp16 automatic-mixed-precision (AMP) layout with the Adam optimiser, fixed memory is 16 bytes per parameter, independent of batch size or sequence length: a 2-byte fp16 parameter used for forward and backward work, a 2-byte fp16 gradient, and three fp32 copies - a 4-byte master parameter that is updated, a 4-byte momentum term, and a 4-byte variance term. Two plus two plus four plus four plus four is sixteen. This is a budgeting estimate, not a universal law.</p>
...
<p><b>Sequence length versus batch size.</b> Activation memory is the piece that does depend on the batch and the input: it grows with batch size, sequence length, model depth, and hidden size, and the attention mechanism's own intermediate values grow with the <i>square</i> of sequence length specifically, since a full attention score matrix is sequence length by sequence length. On a GPU with a fixed memory budget, a longer sequence length forces a smaller batch size to keep total memory in bounds, and because of that squared term, doubling sequence length costs more activation memory than doubling batch size does.</p>`,
        deeper: `<p>Different precision and optimizer recipes keep different copies. Some keep the gradient in fp32, which raises this fp16-AMP estimate from 16 to 18 bytes per parameter (2 fp16 parameter, 4 fp32 gradient, and the same 12 bytes of master parameter plus optimiser state). bf16 and fused or low-bit optimizers can use another layout. Use the 16-byte estimate only when its assumptions match, then inspect the framework and optimizer settings before committing hardware.</p>`,
        check: {
          ...
          answer: 1,
          explain: 'Under the stated 16-bytes-per-parameter estimate, 13,000,000,000 x 16 bytes = 208,000,000,000 bytes, or 208 GB. This is why a 13-billion-parameter model needs its memory split across several GPUs before activations even enter the picture.'
        }
      },
      {
        id: 'tas-f4',
...
<p><b>FlashAttention</b> applies exactly this idea to attention. It tiles the computation so each piece fits in the GPU's fast on-chip memory and the full attention score matrix is never written out to main memory at all, cutting memory traffic sharply without changing the mathematical result. <a href="#llm-inference-engineering/lie-f5">FlashAttention: why it is faster</a> goes through the mechanism in full.</p>
...
<p><b>Checkpoint and restart.</b> A large training run can last days or weeks, and any interruption before the very end would be an unacceptable waste if it meant starting over. Training periodically saves its full state - model weights, optimizer state, the current step number, and random-number-generator state - to durable storage, so a crash loses only the work done since the last checkpoint rather than the whole run. Exact replay additionally needs the data version and sampler/dataloader position, a compatible distributed setup, and deterministic-compatible kernels; a checkpoint normally guarantees resumability, not deterministic replay by itself.</p>
...
<p><b>Tracking</b> means logging every run's configuration, metrics over time, and resulting files to a shared system rather than to scattered notebooks or a person's memory, so results across many experiments can be compared later. <a href="#mlops-tooling/mot-f3">Experiment tracking</a>, in the MLOps (machine learning operations) tooling chapter, covers what a tracker such as MLflow records and how a model registry sits on top of it.</p>
<p><b>Ablations.</b> A one-factor ablation changes one factor against a fixed baseline - one architecture choice, one hyperparameter, one piece of the training pipeline removed or swapped - and measures that factor's effect in isolation. Changing several things between two runs and then comparing them does not identify which single change mattered. When interactions are the question, use a planned factorial or controlled multi-factor study instead. Either approach turns "we tried a few things and it got better" into a specific, defensible claim about what changed the result.</p>`,
    ...
    activities: [
      {
        ...
        prompt: 'You want to fully fine-tune a 3-billion-parameter model in the fp16 AMP plus Adam layout used by the 16-bytes-per-parameter estimate, on a single GPU with 48 GB of memory. Work out whether the fixed memory (parameters, gradients, optimizer state) fits, and if it does not, name one concrete way to make it fit on that one GPU.',
        timeboxSec: 300,
        rubric: `Must-haves: (1) states that the 16-byte arithmetic assumes the fp16 AMP plus Adam layout in the prompt, then computes 3,000,000,000 x 16 bytes = 48,000,000,000 bytes = 48 GB; (2) correctly concludes this does not fit, since 48 GB of fixed memory alone would consume the entire 48 GB GPU with nothing left for activations, let alone anything else running on the machine; (3) proposes at least one concrete, correct fix that keeps the job on one GPU: LoRA or another parameter-efficient method (only a small fraction of parameters need gradients and optimizer state), 8-bit or lower-precision optimizer states, or activation checkpointing (which addresses activation memory specifically, so it should be named alongside a fix for the fixed 48 GB, not instead of one); (4) does not claim ZeRO or FSDP as a single-GPU fix, since both are about sharding across multiple GPUs, not about single-GPU memory reduction. Bonus: notes that even a fix that solves the fixed-memory problem still leaves activation memory to budget for separately. Common mistakes: forgetting the estimate's precision/optimizer assumptions; forgetting to account for activation memory needing room beyond the 48 GB fixed cost; proposing multi-GPU sharding as a fix for a single-GPU constraint; arithmetic errors in the base calculation. {{HONESTY}}`,
        model: `<p>"Under the fp16 AMP plus Adam layout in the question, fixed memory is estimated at 16 bytes per parameter: 3,000,000,000 x 16 = 48,000,000,000 bytes, which is 48 GB. That already equals the entire GPU's memory, with nothing left over for activations, so this does not fit as a full fine-tune, even before a single batch is processed.</p>
<p>Since this is a single-GPU constraint, sharding across multiple GPUs with ZeRO or FSDP isn't available here - that solves a different problem. The fix that actually helps on one GPU is to stop needing gradients and optimizer state for all 3 billion parameters in the first place: LoRA freezes the base weights and only trains a small low-rank adapter, so the optimiser state and gradient only apply to a tiny fraction of the parameters. What is left is mostly the frozen base weights themselves, about 6 GB in fp16 for a 3-billion-parameter model, instead of 48 GB.</p>
    ...
    glossary: [
      {
        ...
        rows: [
          ...
          ['Mixed precision', 'Doing most arithmetic in a lower-precision format such as bf16 or fp16 while keeping selected values in fp32.', 'A common setting for modern large-scale training.'],
          ['16 bytes per parameter', 'A fixed-memory estimate for fp16 AMP with Adam: 2 (fp16 parameter) + 2 (fp16 gradient) + 4+4+4 (fp32 master parameter, momentum, variance). Other precision and optimizer layouts differ.', 'Estimating whether a model fits on given hardware before training starts.'],
          ...
          ['Ablation', 'A controlled comparison that isolates a factor; one-factor studies estimate a main effect, while planned multi-factor studies can test interactions.', 'Attributing an improvement to a specific change.']
        ]
      }
    ]
  };
