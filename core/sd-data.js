...
<li><b>Normalisation.</b> Organising a schema so each current, independently changeable fact has one authoritative place, avoiding copies that can drift. A historical snapshot - the delivery address or price used for one completed order - is a different fact and may belong on that order deliberately.</li>
...
<p><b>Why normalise.</b> First decide what a column means. If every order row copies a customer's <i>current contact address</i>, changing that address requires updating every copy, and one missed row creates an <b>update anomaly</b>. Store that mutable profile fact once in <code>customers</code> and reference it by <code>customer_id</code>. An order's <i>delivery address at the time it was placed</i> is different: it is an intentional historical snapshot and must not change when the customer later moves. Normalisation removes accidental copies of the same fact; it does not erase facts whose time or business meaning differs.</p>
...
<p><b>When to denormalise on purpose.</b> Normalisation optimises for correct, cheap writes; it does not optimise for reads, because a normalised schema often needs several joins to answer one question. When a read path is hot and the joined tables rarely change, copying a value onto the row that needs it - a product's name onto an order line, taken at order time - is a deliberate, documented trade: faster reads, and an explicit decision about what happens if the source value later changes. Say which one you are choosing and why; do not drift into it by accident.</p>`,
        ...
        check: {
          question: 'An orders table copies a customer_email field that is explicitly meant to show the customer\'s current contact email. The customer changes it, and one order still shows the previous value. What is this an example of?',
          options: [
            'A phantom read',
          ...
          answer: 2,
          explain: 'Because the field is defined as the current contact email, every copy represents the same mutable fact and can drift. Store that fact in customers and reference it by customer_id. If the requirement were instead "email used when this order was placed," the old value would be an intentional historical snapshot, not an anomaly.'
        }
      },
      {
        id: 'sql-f2',
...
<p>The reason ACID is asked about constantly, and reasoned about badly just as often, is that people treat it as one guarantee when it is really four separable ones, and the interesting engineering questions live in the gaps between them - specifically, in exactly how strong "isolation" is by default, which the next section covers directly.</p>`,
        deeper: `<p>Consistency is the letter most often misunderstood, because the word is overloaded across this whole subject: the CAP theorem's (Consistency, Availability, Partition tolerance; see <a href="#distributed-coordination/dc-4">CAP and PACELC</a>) "consistency" is about whether every reader sees the same, most recent write; ACID's "consistency" is about whether the database's own rules stay satisfied. They are different claims about different things, and a system can have one without the other - a single-node database with strict ACID consistency says nothing at all about CAP, because CAP only applies once there is more than one node and a network between them.</p>`,
        check: {
      ...
      },
      {
        ...
        body: `<p>The SQL standard names four isolation levels, from loosest to strictest: <b>read uncommitted</b>, <b>read committed</b>, <b>repeatable read</b>, and <b>serializable</b>. Those names specify minimum guarantees, but engines are allowed to provide stronger behaviour at a named level, so always state which database you mean.</p>
...
<p><b>What the standard requires.</b> Read uncommitted may expose dirty, non-repeatable, and phantom reads. <b>Read committed</b> prevents dirty reads but may still return a newer row value or a different matching row set on a later statement. <b>Repeatable read</b> prevents dirty and non-repeatable reads, while the standard still permits phantoms. <b>Serializable</b> requires an outcome equivalent to some serial ordering of the transactions; an implementation may block or abort a transaction and require the application to retry rather than literally run one transaction at a time.</p>
...
<p><b>Locking underneath.</b> A <b>shared lock</b> lets other readers in but blocks writers; an <b>exclusive lock</b> blocks everyone. Two transactions that each hold a lock the other needs produce a <b>deadlock</b>; the database detects the cycle and aborts one side, which the application must be ready to retry.</p>`,
        deeper: `<p>PostgreSQL is a useful example of why the engine matters. Its repeatable read level uses snapshot isolation and prevents phantom reads, which is stronger than the SQL standard's minimum for that level, but it can still permit a serialization anomaly called <b>write skew</b>: two transactions read overlapping facts, update different rows, and jointly violate an invariant. PostgreSQL's serializable level detects these dangerous dependency patterns and aborts one transaction for retry. Other engines attach different locking or snapshot behaviour to the same isolation-level names, so do not generalise PostgreSQL's result without naming it.</p>`,
        check: {
...
<p><b>How it works.</b> A transaction can modify a page in the database's memory buffer before that page is written to its durable data file. The write-ahead rule is about durable write order: the log record describing a change must be flushed before the corresponding dirty data page may be flushed. To acknowledge a commit durably, the database must also flush the transaction's commit record. Data pages can then be written later in an efficient order, because recovery can reconstruct committed changes from the log. The log is commonly much cheaper to flush than all changed pages because it is written largely as a sequential append instead of as scattered page writes.</p>
<p><b>Checkpoints.</b> A checkpoint records recovery progress and enough database state to establish a bounded place from which restart processing can begin. It does not universally mean that every earlier log record's effect has already been copied to its final data page; the exact guarantee and whether old log can be recycled depend on the engine and checkpoint protocol.</p>
<p><b>Crash recovery.</b> On restart, the engine uses the checkpoint and log to redo changes needed to bring data pages up to date, while ensuring uncommitted transactions are not visible. The exact mechanism is engine-specific: ARIES-style recovery can perform explicit undo using log records, while PostgreSQL primarily replays WAL and relies on transaction status and MVCC visibility so tuples from uncommitted transactions remain invisible. Do not assume every database performs the same physical undo pass.</p>
<p><b>What else the log is used for.</b> The same stream of logged changes can be shipped to another machine and replayed there continuously, which is how most relational replication works, and it can be archived and replayed up to any past point in time for point-in-time recovery from a backup.</p>`,
        deeper: `<p>Keep two ordering rules separate. Before a dirty data page is written to durable storage, the log records needed to recover that page must already be durable. Before a durable commit is acknowledged, the commit record must be durable. The in-memory page may change before either flush, and the durable data-page write may lag well behind the commit; violating either durable-ordering rule can make crash recovery lose or misinterpret a change.</p>`,
        check: {
    ...
    activities: [
      {
        ...
        timeboxSec: 1500,
        rubric: `Marking guide, out of 10. (1) A schema is proposed with primary and foreign keys, while explicitly preserving order-time facts such as delivery address and price as intentional snapshots rather than reading mutable current profile values later. (2) Each normalisation or snapshot choice names the fact's meaning and update behaviour. (3) Indexes are tied to actual queries and their write/storage cost is acknowledged. (4) Inventory reservation uses a concrete concurrency-safe mechanism such as an atomic conditional UPDATE or row lock. (5) The answer does not claim a local SQL transaction can atomically include a charge performed by an external payment provider. (6) A durable checkout/payment attempt with a stable idempotency key and pending/unknown states is stored, then reconciled with the provider; failure compensation such as releasing a reservation is addressed. (7) Provider callbacks or outbox delivery are treated as at-least-once, so handlers are idempotent. (8) Read replicas are proposed with replication-lag and read-your-writes trade-offs. (9) Pooling or partitioning is considered before sharding, and scale assumptions are stated. (10) At least one trade-off has both sides named. Deduct for: claiming ACID alone prevents overselling or makes the external charge atomic; promising exactly-once payment without provider idempotency and reconciliation; omitting an inventory concurrency mechanism; sharding before cheaper options. {{HONESTY}}`,
        model: `"I'd use customers, orders, order_lines, inventory, and payment_attempts. An order references customer_id, but it also snapshots the delivery address and each line's unit price used for that purchase. Those are historical order facts, not stale copies of the customer's current profile.
...
For inventory, I'd use an atomic conditional update, UPDATE inventory SET stock = stock - 1 WHERE product_id = ? AND stock >= 1, and check rows affected; zero means sold out. But the external payment call cannot participate in that database transaction. In the local transaction I'd reserve inventory and create a payment_attempt row with a stable idempotency key and pending state. A worker calls the provider with that key, records the result, and safely retries or queries provider status when the outcome is unknown. Provider callbacks and outbox messages can be delivered more than once, so consumers deduplicate by the durable attempt id. If payment definitively fails, a compensating transaction releases the reservation; ambiguous attempts stay pending for reconciliation rather than risking a second charge.
...
I'd also confirm the index it depends on still exists and is usable - something could have dropped it, or a migration changed a column type in a way that stops the planner using it. 'This morning, no code change' points me first at a data or maintenance event overnight, and EXPLAIN ANALYZE turns that suspicion into an answer."`
      },
      {
        ...
        prompt: `Practice problem for PostgreSQL. A reporting job runs one long transaction that reads the same set of "active subscriptions" three times during its run, ten minutes apart, and needs to see the exact same set of rows all three times, even though subscriptions are being created and cancelled continuously elsewhere. Which isolation level do you need, and why doesn't the level below it work?`,
        timeboxSec: 420,
        rubric: `Must-haves: (1) explicitly scopes the answer to PostgreSQL and identifies repeatable read as the minimum level for this read-only requirement; (2) explains that PostgreSQL read committed takes a new statement snapshot, so inserted or cancelled subscriptions can change later results; (3) explains that PostgreSQL repeatable read uses one transaction snapshot and prevents those phantom-row changes; (4) recognises serializable as a valid stronger choice but unnecessary for this pure read, rather than calling it wrong; (5) notes that the SQL standard permits phantoms at repeatable read and other engines may therefore require serializable or an engine-specific snapshot mode; (6) names the cost of retaining old row versions for the long snapshot. Common mistakes: generalising PostgreSQL behaviour to every database; recommending SELECT FOR UPDATE for the read-only report; claiming read committed is sufficient. {{HONESTY}}`,
        model: `"In PostgreSQL, repeatable read is the minimum for this requirement. Read committed takes a fresh snapshot for each statement, so a subscription inserted or cancelled between reads can appear or disappear from the next result.

PostgreSQL repeatable read uses one transaction snapshot, so later commits elsewhere are invisible and all three queries see the same row set. Serializable would also satisfy the requirement, but its extra detection and retry behaviour is unnecessary for this pure read.

That answer is engine-specific: the SQL standard permits phantoms at repeatable read, so another engine may require serializable or its documented snapshot mode for the same guarantee.
    ...
    glossary: [
      {
        ...
        sub: '',
        rows: [
          ['Normalisation', 'Giving each current, independently changeable fact one authoritative place; an intentional historical snapshot is a different fact', 'schema design'],
...
<p><b>Detecting it before it becomes an incident.</b> Per-key or per-partition metrics, not just per-cluster averages, are what actually reveal a hot partition; a cluster-wide utilisation graph can look calm while one partition is silently throttling every request that reaches it.</p>`,
        deeper: `<p>Salting has a real cost worth naming explicitly: each write is routed to one of N salted sub-keys, so aggregate write load can spread across partitions, but reading the complete logical value may require a fan-out across all N sub-keys followed by aggregation. Replicating every write to all N keys would not spread write work; that is a different read-scaling technique. Salting is worth doing only when relieving one hot write key outweighs the more complex and expensive read path.</p>`,
        check: {
...
<p><b>The trade-off, honestly stated.</b> Keyset pagination needs a sort order with no ties (a unique column, or a compound key that breaks ties), it cannot jump to an arbitrary page number the way offset can, and reversing direction (previous page) needs the query logic mirrored rather than just walking backward from an offset. For anything that behaves like an infinite scroll or a feed - where "give me page 47" is never actually asked for, and correctness under concurrent writes matters - keyset pagination is close to always the better choice; for a small, mostly static result set where jumping to an arbitrary page number is a real requirement, offset's simplicity can be worth its cost.</p>`,
        ...
        check: {
          ...
          options: [
            ...
            'New posts inserted at the top shift every later post down by one offset position while the user is mid-scroll, so a post already seen reappears on the next page; keyset pagination anchored to the last (created_at, id) pair seen avoids this',
            'The database\'s index is corrupted',
            'This is expected and cannot be fixed with any pagination scheme'
          ],
          answer: 1,
          explain: 'Offset pagination is positional, so an insert ahead of the current position shifts later rows and causes duplicates or skips. A keyset cursor must contain the full unique sort key - for a newest-first feed, commonly (created_at, id) - unless the id itself is explicitly time-ordered.'
        }
      },
      {
        id: 'npi-f8',
...
<p><b>Hot storage</b> is fast and comparatively expensive per gigabyte - in-memory or SSD-backed, low latency, no retrieval delay or fee - and it is where recent, frequently read data lives. <b>Cold storage</b> is slow and cheap per gigabyte - often object storage with a deliberate retrieval delay measured in minutes to hours, and sometimes a retrieval fee - and it is where data that is accessed rarely, but still needs to be kept, ends up. For example, Amazon S3 offers Standard, Standard-IA (infrequent access), Glacier Instant Retrieval, Glacier Flexible Retrieval, and Glacier Deep Archive; Google Cloud Storage offers Standard, Nearline, Coldline, and Archive. The cheaper archive tiers reduce storage cost but can add retrieval charges or delays, so choose them only when the access pattern tolerates those costs.</p>
...
For partitioning, I wouldn't range-partition by creation time or the roughly time-ordered id, since both would concentrate writes on the newest range. Hashing even a monotonic id would distribute writes, but I'd hash-partition on customer_id because it also keeps one customer's order history local. With consistent hashing, adding capacity later moves a small fraction of the data, and virtual nodes reduce uneven placement.
