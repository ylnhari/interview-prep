...
<li><b>EXPLAIN.</b> The command that shows the plan the optimizer chose (and, with <b>ANALYZE</b>, what actually happened when it ran).</li>
...
<li><b>ACID.</b> Atomicity (all of a transaction's writes happen or none do), consistency (the database's own rules and constraints are never violated), isolation (concurrent transactions do not corrupt each other's view of the data), durability (once committed, a write survives a crash).</li>
...
<li><b>UUID.</b> A 128-bit identifier, usually generated to be globally unique without any coordination between machines.</li>
...
So the false-positive rate is about 0.8 percent, roughly 1 wrong answer in every 122 checks against an absent item - under the 1 percent target, so 10 bits per item is enough. Worth noting: the answer only depends on the bits-per-item ratio, not the raw count of 500,000; the same budget gives the same rate whether there are five hundred thousand items or five million."`
    ...
    sayItOutLoud: null,
    readings: [
      { l: 'PRIMER: ByteByteGo - SQL vs NoSQL: Choosing the Right Database', u: 'https://blog.bytebytego.com/p/sql-vs-nosql-choosing-the-right-database', w: 'A practical decision framework for the choice this chapter opens with, including where NewSQL sits between the two.', m: 10 },
