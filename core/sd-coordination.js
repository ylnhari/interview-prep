...
<p><b>When the cache is down.</b> A cache-aside setup naturally treats an unreachable cache as a permanent miss and falls back to reading the origin directly - which is exactly the problem, because the cache was likely absorbing the great majority of traffic, and the origin was never sized to take all of it at once. This is the same kind of failure as a cache avalanche, and the same tools apply: a circuit breaker in front of the cache so the application fails fast instead of piling up slow calls to a dead cache, request coalescing so concurrent requests for the same key do not each hit the origin separately, and load shedding or rate limiting on the origin so it degrades rather than falls over entirely.</p>`,
      ...
      },
      {
        ...
        body: `<p>Say a generic entity-lookup API gets 20,000 reads per second at peak, each asking for one entity's record. Traffic follows a common pattern: a Zipf-like skew where a minority of entities account for most reads, and a long tail are read rarely. The target is a 95% hit rate, meaning only 5% of reads - 1,000 requests per second at peak - are allowed to reach the origin.</p>
...
<p>What you get is real atomicity across the participants, and isolation along with it: nobody sees a half-finished result, because the locks are held until the second phase completes. What you pay is that those locks are held across at least one network round trip, which limits throughput badly, and that the protocol blocks. If the coordinator dies after collecting the yes votes but before announcing the decision, every participant is stuck holding locks and unable to decide on its own, because it does not know what the others answered. That failure is why two-phase commit is called a blocking protocol, and it is the single most common thing an interviewer wants to hear you name. In practice it is used where the participants are few, close together and under one team's control - the classic case being several relational databases coordinated through the XA standard, which is the interface most database engines expose for exactly this.</p>
...
<p><b>The trade between them.</b> Two-phase commit buys you atomicity and isolation and pays with locks, latency and a coordinator that can block the whole group. A saga buys you throughput, independence and no cross-service locks, and pays by letting other readers see intermediate states and by making you write and test an undo for every step. Put another way: two-phase commit asks the infrastructure to hide the problem, and a saga asks the business logic to handle it in the open. Because sagas hold no locks, they scale across many services and across regions where two-phase commit realistically cannot, which is why microservice architectures reach for them by default.</p>
<p><b>The answer that is usually better than either.</b> Most distributed transactions exist because data that changes together was split apart. If the two writes really must be atomic, the strongest design is to keep them in one service's database and use one ordinary local transaction. Where the split is genuine, the <b>transactional outbox</b> removes the most common need for a distributed one: the service writes its business change and a row describing the event it wants to publish in the same local transaction, and a separate process reads that table and publishes the event. The commit and the announcement can then never disagree, because they were one commit.</p>`,
