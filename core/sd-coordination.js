...
<p><b>Gateway or reverse-proxy cache.</b> Sitting just in front of the application servers (for example, in a load balancer, an API gateway, or a tool like Varnish or NGINX), this cache solves load on the application servers themselves: identical requests are answered without ever running the application's own code.</p>
...
<p><b>Write-behind (write-back).</b> A write is written to a cache-side buffer and acknowledged to the application before the origin is updated, often so several writes can be batched. This makes writes fast but creates an interval in which the acknowledged value exists only in that write-behind tier. A volatile, unreplicated buffer loses pending writes if its node crashes; a replicated or durably logged buffer can survive that node loss, but it adds coordination and still needs ordering, retry, deduplication, and recovery before it can offer a meaningful durability guarantee.</p>
<p><b>Choosing between them.</b> Write-through is the simpler default whenever losing an acknowledged write is unacceptable. Write-behind earns its complexity when write volume is high and the origin is slow or expensive per write, provided the system either accepts a bounded loss window or explicitly engineers the pending-write buffer for the required durability. View counts may accept loss; payments and orders normally require a durable queue or log plus reconciliation rather than a volatile cache buffer.</p>`,
        ...
        check: {
          question: 'A system uses write-behind with a volatile, unreplicated buffer for a counter. The cache node holding pending increments crashes before flushing to the database. What is lost?',
          options: [
          ...
          answer: 1,
          explain: 'Because this buffer is explicitly volatile and unreplicated, acknowledged increments not yet flushed to the origin are lost. A durably logged or replicated write-behind queue could survive a node failure, at the cost of additional coordination and recovery machinery.'
        }
      },
      {
        id: 'cache-4',
...
<p><b>Event-driven invalidation.</b> When a write happens at the origin, it publishes an event (directly, through a message queue, or through a <b>change-data-capture</b> stream - a feed of row changes emitted from the database log) naming the affected keys. If that event is delivered and processed by every cache that can serve the key, the next read there misses and refetches. A lost, delayed, or out-of-order event breaks that immediate-freshness expectation, which is why TTL remains a useful backstop.</p>
<p><b>Versioned keys.</b> Bake a version into the data key, such as <code>product:42:v17</code>, so a changed value gets a new key and old cached content is not overwritten. This only works if the reader can learn that version 17 is current, for example from authoritative metadata, a consistently updated pointer, or a delivered change event. That version lookup is itself a cache/coordination path and can become stale. Once a reader has the new version, its first data lookup misses and populates the new key; old versions expire or are evicted.</p>
<p><b>Why invalidation is hard.</b> A cache is often replicated across several nodes, so "invalidate this key" has to reach every node that might be holding it. A lost or delayed event leaves stale data until another mechanism removes it, and an older event arriving after a newer one can overwrite or delete the wrong version unless events carry versions and consumers handle duplicates and reordering. There is also a race between the origin write and invalidation delivery, and a system can have browser, CDN, and application caches with separate paths. Keep a TTL backstop even when invalidation is event-driven.</p>`,
        deeper: `<p>Versioned keys avoid reliably deleting old data entries everywhere, but they do not eliminate invalidation altogether: the current-version pointer still has to become visible to readers. Old versions also occupy space until TTL or eviction reclaims them. The pattern is strongest when version discovery is authoritative or reliably updated and both pointer and data entries retain bounded TTLs.</p>`,
        check: {
...
<p><b>When the cache is down.</b> A cache-aside setup naturally treats an unreachable cache as a permanent miss and falls back to reading the origin directly - which is exactly the problem, because the cache was likely absorbing the great majority of traffic, and the origin was never sized to take all of it at once. This is the same kind of failure as a cache avalanche, and the same tools apply: a circuit breaker in front of the cache so the application fails fast instead of piling up slow calls to a dead cache, request coalescing so concurrent requests for the same key do not each hit the origin separately, and load shedding or rate limiting on the origin so it degrades rather than falls over entirely.</p>`,
      ...
      },
      {
        id: 'cache-9',
        part: 'field',
        title: 'Worked example: sizing a cache for a read-heavy entity API',
        body: `<p>Say a generic entity-lookup API gets 20,000 reads per second at peak, each asking for one entity's record. Popularity is highly concentrated: a minority of entities account for most reads, while a long tail are read rarely. The target is a 95% hit rate, meaning that in steady state only 5% of reads - 1,000 requests per second at peak - reach the origin.</p>
...
<p><b>Step 2: size per entry.</b> Say each entity record serializes to 1.5 KB. Redis also carries bookkeeping per key - the exact number depends on encoding and allocator behaviour, but budgeting roughly 100 bytes of overhead per key is an illustrative planning assumption. That puts each entry at about 1.6 KB for this estimate.</p>
...
<p><b>Step 4: what the origin still needs to handle.</b> At a steady-state 95% hit rate, cache misses contribute about 1,000 origin reads per second, plus writes and background work. That is the normal-load estimate, not a safe failure capacity: a cold or unavailable cache can expose the origin to as much as the full 20,000 reads per second. The design needs explicit degraded-mode protection - rate limits, load shedding, stale serving where safe, request coalescing, and controlled warming - and enough origin headroom for the failure policy chosen.</p>
<p><b>Step 5: sanity-check the TTL.</b> If entity records are refreshed hourly upstream, a TTL of 10 to 15 minutes keeps served data reasonably fresh relative to how often it changes, without pushing the miss rate up by refetching far more often than the source changes.</p>`,
        ...
        check: {
          question: 'In the worked example, why does the origin see about 1,000 cache-miss reads per second in steady state, and what changes during cache failure?',
          options: [
            'Because the origin is faster than the cache',
            'Because a 95% steady-state hit rate sends the remaining 5% - about 1,000 reads per second - to the origin, but a cold or failed cache can expose it to as much as the full 20,000 unless degraded-mode controls protect it',
            'Because the cache handles writes as well as reads',
            'Because 1,000 requests per second is an industry standard limit'
          ],
          answer: 1,
          explain: 'Hit rate gives the steady-state miss load: 5% of 20,000 is 1,000. It does not cap failure load. Capacity planning must also account for cold start or cache loss with headroom and controls such as coalescing, rate limits, load shedding, stale serving, and gradual warming.'
        }
      }
    ],
    connect: null,
...
For Redis, cache-aside for reads: check Redis, and on a miss, read the database and populate Redis. For writes, write-through rather than a volatile write-behind buffer - stock and price are exactly the data where I don't want an acknowledgement to precede durable origin storage. A replicated or durably logged write-behind system could reduce that loss risk, but its extra machinery is not justified at only 200 writes a second.

I'd set a short TTL, maybe 30 to 60 seconds, since stale stock or price is a visible problem. But I wouldn't rely on TTL alone: on every price or stock write, I'd publish an idempotent, version-ordered invalidation. Successfully processed invalidations make the next read miss; the TTL bounds staleness when an event is delayed or lost.
    ...
    glossary: [
      {
        ...
        rows: [
          ...
          ['Write-behind', 'A write is acknowledged before origin persistence and flushed later; durability depends on whether the pending-write buffer is volatile, logged, or replicated', 'write patterns'],
          ...
          ['Redis hash slots', 'Redis Cluster divides one cluster-wide keyspace into 16,384 fixed slots assigned among primary nodes, rather than using a hash ring directly', 'distributed cache design']
        ]
      }
    ]
  };
...
<li><b>Replication.</b> Keeping copies of the same data on more than one node, for durability or read capacity. Three replicas can preserve data through two replica losses only if all three contained the acknowledged data and failures do not share a common storage or placement fault; it does not automatically keep reads or writes available with two nodes down.</li>
...
<p><b>Clocks.</b> Each machine keeps its own physical clock, built from a quartz crystal that drifts - gains or loses time - at a small but real rate, and even frequent synchronization only bounds that drift, it does not eliminate it. Two machines' clocks are never perfectly identical at the same instant, which means you cannot safely say "event A happened before event B" just because A's timestamp is smaller than B's, if A and B happened on different machines. This is the single most common mistake in distributed system design: treating wall-clock timestamps from different machines as if they gave a reliable global order of events.</p>`,
      ...
      },
      {
        ...
        body: `<p>A consistency model is a precise promise about what operations may observe and how their order relates to other operations. Some global models can be compared by strength, but they are not one universal price ladder: read-your-writes and monotonic reads are session guarantees on a different axis, and the implementation and workload determine the coordination and latency cost.</p>
...
<p><b>Raft, in three parts.</b> <b>Leader election:</b> after a randomized timeout, a follower becomes a candidate and requests votes for a numbered <b>term</b>. A voter grants at most one vote per term and rejects a candidate whose log is less up to date than its own, comparing the candidate's last log term and then last log index. A candidate with a majority becomes leader. <b>Log replication:</b> the leader appends client operations and sends them to followers, which also check that preceding log entries match. <b>Commit:</b> a leader advances its commit index using majority replication for an entry from its current term. Once that current-term entry is committed, earlier entries before it are committed as well; an older-term entry is not declared committed merely because a current leader notices it on a majority. Future leaders retain committed entries because election quorums overlap <i>and</i> the log-freshness voting rule prevents a node missing the committed suffix from collecting the necessary votes.</p>
...
<p><b>Where ZooKeeper and etcd fit.</b> These are not consensus algorithms themselves; they are coordination services built on top of one. ZooKeeper uses its own consensus protocol, ZAB (ZooKeeper Atomic Broadcast), internally, and exposes higher-level primitives on top of it - a small hierarchical namespace, watches that notify a client when something changes, and nodes that automatically disappear if the client holding them dies - which applications use to build locks, leader election, and configuration storage without implementing consensus themselves. etcd uses Raft directly and exposes a simple, strongly consistent key-value store; it is best known as the component Kubernetes itself uses to store all cluster state.</p>`,
        ...
        check: {
          ...
          options: [
            ...
            'Because majority replication combines with Raft\'s voter log-freshness rule: a future candidate missing the committed suffix cannot collect the votes needed to become leader',
          ...
          answer: 1,
          explain: 'Majority overlap alone is not enough: the overlapping voter might have the entry without the candidate having it. Raft also makes voters reject candidates with less up-to-date logs. Together with the current-term commit rule, that produces leader completeness across elections.'
        }
      },
      {
        id: 'dc-6b',
        ...
        body: `<p>Inside one database a transaction is easy to reason about: it all happens or none of it does. The moment the work spans two databases, or two services that each own their own, that guarantee is gone, and "charge the card, then reserve the seat" can leave you having done exactly one of the two. There are two established answers, and an interviewer asking about distributed transactions is nearly always asking you to compare them.</p>
<p><b>Two-phase commit.</b> One node acts as the <b>coordinator</b> and the others as <b>participants</b>, and the commit happens in two rounds.</p>
<ul>
<li><b>Phase one, prepare.</b> The coordinator asks every participant whether it can commit. Each participant does the work, writes enough to its own log to be able to commit later even after a crash, holds its locks, and answers yes or no. A participant that answers yes has given up the right to change its mind.</li>
<li><b>Phase two, commit or abort.</b> If every participant said yes, the coordinator records the decision and tells everyone to commit. If any said no, it tells everyone to abort. Participants apply the decision and release their locks.</li>
</ul>
<p>What two-phase commit coordinates is the <b>atomic commit decision</b>: all prepared participants commit or all abort. It does not by itself choose transaction isolation or prevent every observer from seeing a mixed state; that depends on each resource manager's isolation level, locks, and how readers access the data. Prepared database transactions commonly hold locks until the decision, which can support isolation but also limits throughput. The protocol can block: if the coordinator dies after collecting yes votes but before participants learn the durable decision, prepared participants may remain in doubt and hold resources until recovery. XA is a common interface for coordinating supporting resource managers.</p>
<p><b>The saga pattern.</b> A saga gives up on doing the whole thing atomically. Instead it runs a sequence of ordinary local transactions, one per service, each of which commits on its own immediately; and for each step it defines a <b>compensating action</b> that undoes that step's effect. If step four fails, the saga runs the compensations for steps three, two and one, in reverse order, until the system is back in a state everyone agrees is acceptable.</p>
<ul>
<li><b>Two ways to drive it.</b> With <b>orchestration</b>, a single coordinator service holds the sequence, calls each step, and decides when to compensate; the flow is written down in one place and is easy to follow, at the cost of a component every step depends on. With <b>choreography</b>, each service listens for the previous step's event and emits its own; there is no central component, but the flow exists only as the sum of the subscriptions, and nobody can read it in one place.</li>
<li><b>Compensation is not rollback.</b> A rollback erases a change that was never visible. A compensation is a new, visible business action that offsets an old one: a refund rather than an unmade payment, a cancellation notice rather than an unsent booking. It must be written deliberately, and it can fail, so it needs to be retryable and safe to apply twice.</li>
<li><b>Some steps cannot be compensated.</b> An email that has been sent cannot be unsent, and money paid out to a third party may not come back. The design response is ordering: do the steps that are hard to undo last, once everything reversible has already succeeded.</li>
</ul>
<p><b>The trade between them.</b> Two-phase commit buys an atomic decision and pays with prepared resources, latency, reduced availability, and possible blocking; end-to-end isolation remains a separate design property. A saga avoids cross-service prepared locks and pays by exposing committed intermediate states and requiring explicit, retryable compensation. Because sagas do not hold a transaction open across services, they are often a better operational fit across independent services or regions.</p>
<p><b>The answer that is usually better than either.</b> If two writes must be atomic, keeping them in one service's database permits one local transaction. Where a split is genuine, a <b>transactional outbox</b> atomically commits the local business change and an outbox row. A separate publisher then delivers the event, normally with at-least-once semantics: it may publish the same row again after a crash or lost acknowledgement. Consumers therefore need an event ID and idempotent handling or deduplication. The outbox closes the local commit-versus-event-loss gap; it does not make downstream side effects exactly once.</p>`,
        deeper: `<p>The distinction worth having ready is between atomicity and isolation, because a saga gives you a weakened version of the first and none of the second. Every step commits as it goes, so anyone reading the data mid-saga sees a real, committed, intermediate state - an order that is paid but not yet confirmed. The usual answer is a status field that the rest of the system understands and respects, sometimes called a semantic lock: the record is visible, but its status says it is not yet available to be acted on. That is a design obligation the two-phase version would have handled for you, and forgetting it is where most saga bugs live.</p>
<p>The second thing to say is what happens when a compensation itself fails. There is no deeper layer to fall back on, so the saga needs the same machinery any asynchronous pipeline needs - retries with a cap, and somewhere for a message that keeps failing to go (<a href="#queues-and-streams/qs-f5">Queue lag and backpressure</a>) - and, at the end of it, a human process for the small number of cases that end up stuck. Saying that out loud is a strong answer, because it shows you have thought past the path where every step succeeds.</p>`,
        check: {
          question: 'A booking flow spans three services that each own their own database, and the team chooses a saga rather than two-phase commit. What do they have to accept in exchange?',
          options: [
            'That the flow can no longer be retried once any step has failed',
            'That other readers can see committed intermediate states, such as a booking that is paid but not yet confirmed, so each step needs a status the rest of the system respects and each undo has to be written as a compensating action',
            'That all three services must share a single database connection',
            'That the three services must be deployed together as one release'
          ],
          answer: 1,
          explain: 'A saga trades isolation for throughput. Each step commits immediately, so intermediate states are visible and there is no rollback to fall back on - only compensating actions the team has to write, test and make safe to retry.'
        }
      },
      {
        id: 'dc-7',
...
<p><b>The boundary of the guarantee.</b> Fencing only protects operations whose destination atomically checks the token. It cannot retract an email or stop an external API that ignores the token. Those effects need the receiver's idempotency support, a durable outbox and deduplication, or explicit reconciliation; otherwise the job remains at-least-once and duplicate effects are possible.</p>
    ...
    activities: [
      {
        ...
        viz: 'fencing-token',
        prompt: `"We have a job scheduler running on three replicas. A job may be attempted more than once after failures, but stale or overlapping attempts must not cause duplicate accepted effects - including when one replica is paused by a long garbage collection cycle. Design the coordination and output handling."`,
        timeboxSec: 600,
        rubric: `Must-haves: (1) proposes a lease-based lock with a concrete renewal policy; (2) identifies the pause-longer-than-lease failure mode; (3) uses a monotonically increasing fencing token checked atomically by each protected store; (4) gives the job or run a durable identity and makes retries idempotent or transactional; (5) states the limit: a side effect such as email or an external API call cannot be fenced unless that receiver participates by checking the token or an idempotency key, so use an outbox, provider idempotency, reconciliation, or accept at-least-once effects; (6) does not claim the lock service alone guarantees exactly-once execution. Common mistakes: a lock with no expiry; describing a token without naming the enforcing resource; promising exactly once for unfenceable external effects. {{HONESTY}}`,
...
The fix for fenceable outputs is a fencing token. Every lease grant comes with a number that only increases. The replica includes that token in an atomic write to the output store, which rejects a token lower than the highest already accepted for that job. I also give the scheduled run a durable job/run id and make output writes idempotent, so a retry with the same identity does not duplicate the accepted result.

The lease coordinates who may try; the resource's token check decides which effects count. If the job sends email or calls an external API that cannot check the fencing token, the lock cannot provide exactly once. I would use a transactional outbox plus the receiver's idempotency key where available, and reconciliation for ambiguous results; otherwise I must explicitly accept possible duplicate external effects."`
      },
      {
        id: 'dc-a2',
        type: 'explain',
        ...
        timeboxSec: 480,
        rubric: `Must-haves: (1) identifies that continuing to answer on both sides of a partition sacrifices a single linearizable view for those operations; (2) connects stale reads to replicas that could not receive the latest write; (3) uses PACELC only to ask or explain the separate no-partition policy: does the system normally coordinate for consistency or answer a nearer replica for latency?; (4) does not infer that an availability choice during this incident proves the system's normal PACELC else-branch choice; (5) distinguishes CAP consistency from ACID consistency. Common mistakes: treating the incident as proof of normal-operation settings; describing expected partition behaviour as necessarily a bug. {{HONESTY}}`,
...
PACELC asks a separate question that this incident does not answer: when there is no partition, does the system wait for coordination to preserve stronger consistency, or answer a nearby replica for lower latency? I would inspect the configured read consistency and measure normal replica lag before classifying that else branch. A system can choose availability during partitions yet stronger consistency in normal operation, or make a latency trade in both cases.
...
If a few seconds of staleness during a rare partition is acceptable for most reads, this system is behaving correctly. Any operation that truly can't tolerate staleness is the one I'd single out for a quorum-based or leader-routed read path instead."`
      },
      {
        ...
        timeboxSec: 420,
        rubric: `Must-haves: (1) states the assumptions behind the simplified quorum exercise: all seven replicas are eligible, an operation succeeds after W or R responses, and W + R > N is being used for read/write-set overlap; (2) derives write availability with two replicas down as W <= 5; (3) chooses the largest allowed W, namely W = 5, because the stated objective is to minimise R; (4) derives R > 7 - 5, hence minimum R = 3; (5) distinguishes availability from durability: W = 5 remains writable with two unreachable replicas, while surviving two later losses among acknowledged copies also assumes the write reached five distinct failure-independent replicas; (6) states that R = 3 responses is the read cost and names the trade-off of slower/less available writes for cheaper reads. Common mistakes: choosing the balanced W = 4, R = 4 despite the explicit read-minimisation goal; using W + R = N; omitting placement/failure assumptions. {{HONESTY}}`,
        model: `"I'll assume all seven replicas are eligible, a write succeeds after W distinct acknowledgements, a read waits for R distinct responses, and W + R > N is the overlap rule this exercise wants.

With two replicas unreachable, only five can acknowledge, so write availability requires W <= 5. To minimise read cost I choose the largest W that still satisfies that constraint: W = 5. Then 5 + R > 7 means R > 2, so the minimum is R = 3.

So the answer is W = 5, R = 3, and each read waits for three replica responses. This shifts cost and failure sensitivity toward writes to make reads cheaper. The durability wording also needs an assumption: five acknowledgements place the write on five distinct replicas, so losing any two of those copies still leaves three, provided failures are independent and acknowledgements mean the data reached the promised durable tier."`
      }
    ],
    sayQuestion: null,
    sayItOutLoud: null,
    ...
    glossary: [
      {
        ...
        rows: [
          ...
          ['Majority commit', 'A Raft leader directly advances commit using a majority-replicated entry from its current term; earlier entries then become committed too', 'consensus'],
