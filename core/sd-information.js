...
<li><b>Token / tokenisation.</b> A token is one piece produced by splitting raw text; tokenisation is that splitting step, deciding where one word ends and the next begins. A language model's token is a separate idea that shares the name - a word piece the model reads or writes one at a time; see <a href="#llm-inference-engineering/lie-f1">How a token is generated</a>.</li>
...
<li><b>Stemming.</b> Applying mechanical rules that strip or replace word endings so related forms may share a stem - for example, a stemmer may reduce "connected" and "connecting" to "connect."</li>
<li><b>Lemmatisation.</b> Using vocabulary and grammatical information to map an inflected word to its dictionary form, so the irregular form "ran" can become "run."</li>
...
<li><b>Document frequency (df).</b> The number of documents that contain a given term at least once - a term sitting in 900,000 of a million documents barely narrows a search at all.</li>
<li><b>Term frequency (tf).</b> The number of times a given term appears inside one specific document - a page repeating "cheap" fifty times has a high tf, which is exactly why BM25 (Best Match 25, the standard keyword-ranking formula, defined four entries below) caps how much repetition can help.</li>
...
<li><b>BM25.</b> The standard relevance-ranking formula used by most text search engines today; it behaves like TF-IDF but caps how much repeating a word helps and adjusts for document length. The "25" is historical, from an early numbered series of experimental ranking formulas, not something worth decoding.</li>
...
<li><b>Precision at k.</b> Of the top k results shown, the fraction that are actually relevant - six good results in the top ten is a precision at 10 of 0.6.</li>
<li><b>Recall.</b> Of all the relevant documents that exist, the fraction the search actually returned somewhere in its results - you can always reach perfect recall by returning everything, which is why it is never judged on its own.</li>
...
<li><b>MRR (mean reciprocal rank).</b> The average, across many queries, of one divided by the rank of the first relevant result - a first good hit at rank 2 contributes 0.5, at rank 10 only 0.1.</li>
...
<li><b>Sharding (document vs term partitioning).</b> Two ways to split an index; see <a href="#nosql-partitioning-ids/npi-f3">Sharding and partitioning: range vs hash</a> for the general mechanism and <a href="#search-and-retrieval/sr-f6">Keeping the index in sync and sharding it</a> for the search-specific choice.</li>
<li><b>Embedding.</b> A vector that stands for meaning, so a query for "car" can match a document that only ever says "automobile"; see <a href="#deep-learning-essentials/dle-f9">Embeddings, generative models, and choosing an architecture</a>.</li>
...
<li><b>HNSW / IVF.</b> Two common approximate nearest neighbour index types. HNSW, hierarchical navigable small world, is a layered graph you descend greedily. IVF, inverted file index, clusters the vectors first and then searches only the nearest clusters.</li>
...
<li><b>Crawl frontier.</b> The queue of URLs a crawler has discovered but has not yet fetched, ordered so that one site is not fetched faster than it can stand.</li>
</ul>`,
      ...
      },
      {
        ...
        viz: 'inverted-index',
        body: `<p>Building an inverted index starts with turning raw text into terms. <b>Tokenisation</b> splits a document into candidate words - by whitespace and punctuation, with real complications like "don't" or languages with no spaces between words. Every token is usually lowercased and common <b>stop words</b> like "the" and "is" are often dropped. <b>Stemming</b> applies mechanical ending rules so forms such as "connected" and "connecting" may share a stem, while <b>lemmatisation</b> uses vocabulary and grammar to map an irregular form such as "ran" to the dictionary form "run." Either normalization can make related forms point at the same posting list.</p>
...
<p><b>Online interleaving</b> tests two ranking algorithms against real users at the same time: it mixes results from both algorithms into a single list a user sees, then counts which algorithm's results got clicked more. Because both rankings are shown to the same user in the same session, interleaving needs far less traffic than a traditional A/B test to detect a real difference, since it removes user-to-user variance from the comparison.</p>`,
        ...
        check: {
          question: 'A search engine returns only one result for a query, and that result is highly relevant. Which metric best exposes the hidden problem that the engine may have missed other relevant documents?',
          options: [
...
<p>A team can trade this delay directly: shortening the refresh interval makes writes searchable sooner but increases CPU and I/O load from more frequent, smaller segments; lengthening it reduces that load but means a user can search for something that was just written and not find it yet.</p>`,
      ...
      },
      {
        ...
        body: `<p>An <a href="#deep-learning-essentials/dle-f9">embedding</a> is a vector that stands for the meaning of a piece of text, so two texts about the same thing sit near each other even when they share no words at all - which is exactly what BM25 cannot do. Finding the closest stored vectors to a query vector exactly would mean comparing against every stored vector, which is too slow at scale, so systems use <b>approximate nearest neighbour (ANN)</b> search instead.</p>
...
<li><b>Hopping window.</b> A fixed-length window that advances by a step smaller than its own length, so consecutive windows overlap - a 5-minute window emitted every minute.</li>
...
<li><b>Quantile / percentile.</b> A value below which a given fraction of the data falls - the 99th percentile (p99) is the value below which 99% of observations sit, so it describes the slow tail of a distribution rather than the average. p999 is the same idea one step further into that tail: the value below which 999 of every 1,000 observations sit.</li>
<li><b>Quantile sketch.</b> A sketch that estimates percentiles of a large or streaming dataset without storing every value, which is how a dashboard can show p99 latency without keeping every request timing.</li>
...
<p>That is the definition. It is worth walking one window through in numbers, because the mechanism is easier to hold on to than the wording.</p>
<p>Say you count card transactions per minute, and you set the watermark to trail the newest event time you have seen by <b>30 seconds</b> - a figure you got by measuring how far behind your slowest source actually runs, not by guessing. The 14:32 window covers events whose event time falls between 14:32:00 and 14:32:59. The processor does <i>not</i> close that window at 14:33:00. It closes it when the watermark reaches 14:33:00, which happens around 14:33:30 by the wall clock, and everything that turned up during those extra thirty seconds is still counted. So an event stamped 14:32:41 that arrives at 14:33:20 is perfectly on time - the window was still open. The same event arriving at 14:35:00 is late, and what happens to it is a decision you made in advance:</p>
<ul>
<li><b>Drop it.</b> The count for 14:32 stays exactly as emitted. This is the simplest option and it is the right one whenever the number has already been used for a decision that cannot be taken back.</li>
<li><b>Allow lateness and emit again.</b> Keep the window's state for a further fixed period - ten minutes, say - and publish a corrected count for 14:32 when a late event lands. The cost is downstream: every consumer must now cope with receiving a second, different answer for the same window, which means being keyed by window rather than appending, or being idempotent.</li>
<li><b>Route it to a side output.</b> Send late events to a separate stream or table, leave the original result untouched, and reconcile in a batch job later. This is the usual choice when the fast path feeds decisions and a nightly job produces the numbers of record.</li>
</ul>
<p>Two sizings are worth remembering because interviewers ask for them. The <b>watermark delay</b> should come from measuring your real source lag - the ninety-ninth percentile of processing time minus event time - rather than from a round number: too small and you spend your life emitting corrections, too large and every result is late by that amount even on a day when nothing goes wrong. The <b>allowed lateness</b> is a separate and usually larger number, and it costs memory rather than latency: the processor has to hold the state of every open window for that whole period, so "allow one day of lateness" on a per-user session window is a memory bill, not a free improvement in correctness.</p>
<p>The one thing that is never a valid option is to put the event into whichever window happened to be open when it arrived. That is counting by processing time, and it reports that a transaction happened at 18:00 when it happened at 14:32.</p>`,
...
<p>Two follow-ups come up often enough to be worth having ready.</p>
<p><b>What happens when there is no traffic?</b> A watermark normally advances because new events keep arriving with later timestamps. So a partition that goes quiet stops advancing its own watermark, and because the job's watermark is the minimum across all its sources, one idle partition can freeze every window in the job behind it - results simply stop appearing, with no error and no failed health check. Stream processors handle this with an idle-source timeout that lets a quiet partition's watermark advance on the wall clock instead. That is the right fix, and it reintroduces exactly the risk you were avoiding: closing a window moments before a delayed burst arrives. Knowing that this problem exists is a strong signal that someone has operated a streaming job rather than only read about one.</p>
<p><b>What does a corrected result mean for anything downstream?</b> Once you allow lateness, a window result is no longer a fact, it is the current best answer. A dashboard can simply redraw. A model feature cannot: the value the model actually read at decision time and the corrected value are two different numbers, and training on the second one teaches the model to expect information it will never have. Keeping both, and knowing which one belongs where, is the whole subject of <a href="#data-features/df-v2-transformations-replay">Features computed on a stream</a>.</p>`,
        check: {
      ...
      },
      {
        ...
        body: `<p>HyperLogLog estimates how many distinct items a stream has contained, using a fixed, tiny amount of memory no matter whether the true count is a thousand or a billion. The intuition: hash every incoming item to a long, effectively random binary string, and split that hash into two parts. The first few bits pick one of <b>m</b> buckets; the remaining bits are scanned for their run of leading zeros. Each bucket stores <b>rho</b>, the one-based position of the first 1 bit: two leading zeros therefore produce a register candidate of 3. This is the longest-leading-zero-run idea plus one, not a different signal. A long run is rare - a run of k leading zeros should only turn up about once every 2^k hashes - so if a bucket has seen a long run, that bucket has probably processed many distinct items. Combining the register values across all m buckets (using a specific averaging formula, not a plain mean) gives an estimate of the total distinct count.</p>
...
<p><b>Worked example:</b> with a reservoir of size k = 3, when the 5th item arrives, the probability it replaces something in the reservoir is 3/5, and if it is chosen to replace, each of the 3 existing slots is equally likely (1/3 each) to be the one it replaces.</p>`,
      ...
      },
      {
        ...
        viz: 'tdigest-merge',
        body: `<p>A percentile is not a quantity you can average across machines and get a correct answer: if one region reports a p99 latency of 50ms and another reports 500ms, the true combined p99 across both regions' traffic is not the average of those two numbers. Under the percentile definition used here, the combined p99 must lie between 50ms and 500ms inclusive, but its exact position depends on the full distributions and how much traffic each region served. The mathematically correct way to combine them is to merge the underlying data, or mergeable sketches of it, and compute the percentile once on the combined result.</p>
<p>If two regions have the same <em>exact</em> p99 under the same percentile definition - 120ms in both - their combined exact p99 is also 120ms. Two dashboards can still display the same rounded or approximate p99 even when the underlying quantiles differ, so their merged sketches may produce a different displayed value. That is an approximation or rounding nuance, not permission to average percentiles.</p>
    ...
    activities: [
      {
        ...
        timeboxSec: 360,
        rubric: `Must-haves: (1) explicitly rejects an exact, single-counter approach for either number and explains why (contention on shared state at this scale); (2) proposes a workable mechanism for the reaction total - local buffering/batching per server merged periodically, described as an approximate, refreshed-every-second-or-two number rather than an instantaneous exact one; (3) defines "watching now" as recent activity, deduplicates with a stable viewer id, and uses active connection gauges or an expiry-aware structure such as rotating HyperLogLog time buckets; (4) explains that a cumulative HyperLogLog cannot represent current membership because its registers cannot delete viewers, and merges or rebuilds only the buckets still inside the activity window. Common mistakes: proposing an exact atomic counter for either number with no acknowledgement of contention; treating a cumulative HyperLogLog as if viewers disappear when they disconnect; using a new session id on every reconnect and double-counting one viewer. {{HONESTY}}`,
...
For 'people watching now,' I would first define "now" as a stable viewer id that has sent a heartbeat in the last 30 seconds, so reconnects do not create a new person. A simple operational estimate is the sum of per-gateway active-connection gauges when a viewer can own only one connection. If distinct viewers can appear on several gateways, each gateway writes stable viewer ids into fresh HyperLogLog time buckets; the aggregator merges only buckets that overlap the 30-second window and rebuilds as old buckets expire. A cumulative HyperLogLog would be wrong because its registers can increase but cannot delete viewers who left. Around 1% standard error is fine for a number shown to users.
...
So it's not that one is better - it's a genuine trade of memory for exactness and enumerability, and I'd only make that trade once the key space is actually too large for a hash map to be practical, not as a default choice."`
      },
      {
        ...
        prompt: `Interviewer: "One regional service reports p99 latency of 40ms and another reports 400ms. What is the fleet-wide p99?"`,
        timeboxSec: 150,
        rubric: `Must-haves: (1) says the exact fleet-wide p99 is unknowable from the two p99 values alone and, under the percentile definition used here, bounds it between 40ms and 400ms inclusive; (2) explains why averaging, maxing, or otherwise arithmetically combining already-summarised percentiles is invalid; (3) explains that the distributions or mergeable sketches must be merged before recomputing p99, naming t-digest or DDSketch. Common mistakes: claiming the answer is 220ms or 400ms; claiming the combined exact p99 can fall outside the two component exact p99s under this definition; describing the problem without a concrete mergeable mechanism. {{HONESTY}}`,
        model: `"I cannot determine the exact fleet-wide p99 from those two regional p99 values alone. Under the percentile definition we are using, it must lie between 40ms and 400ms inclusive, but its location in that interval depends on both full latency distributions and how much traffic each region served. Averaging to 220ms or taking the maximum at 400ms would both invent information the summaries discarded.
...
<li><b>Pub/sub.</b> One message to every listener; see <a href="#queues-and-streams/qs-f2b">Publish/subscribe: the third messaging model</a>.</li>
...
<li><b>Fan-out on write (push model).</b> Delivering a new post into every follower's feed at the moment it is created, so reading a feed is one cheap lookup and the whole cost lands on the person posting.</li>
<li><b>Fan-out on read (pull model).</b> Building a feed at the moment it is requested, by merging recent posts from everyone the requester follows, so writes stay cheap and every feed load pays for the merge.</li>
...
<p>As a rough guide: reach for short polling only when updates are infrequent and a real-time feel does not matter; long polling when true push infrastructure is not available but wasted requests matter; SSE when the server only ever needs to push to the client (a live score feed, a notification stream); and WebSockets when both sides need to send messages on one connection.</p>`,
      ...
      },
      {
        ...
        body: `<p>A single established WebSocket is one TCP connection owned by the server that accepted it - two servers cannot share that open socket. After the HTTP upgrade, its packets already follow that connection to the same backend, so load-balancer stickiness is not what keeps the live socket attached. Affinity can still be useful for a reconnect or related HTTP requests when local state matters, but a reconnect may land on any healthy server and must register its new owner.</p>
<p>If server A needs to deliver a message to a user connected to gateway B, a <b>connection registry</b> - commonly a shared store mapping user id and connection id to gateway id - answers who owns the socket. Delivery then goes through <a href="#queues-and-streams/qs-f2b">pub/sub</a> to a gateway or shard channel for B. Only the owning gateway or shard needs that channel; it forwards the message down the specific local socket. This avoids subscribing every server to every per-user channel.</p>
<p><b>Heartbeats</b> - small ping/pong messages sent periodically - catch a connection that has gone silently dead so the server can free the slot. The registry entry should also be a renewable lease so it expires if the gateway dies before cleaning it up. On the client side, <b>reconnecting with backoff and jitter</b> - waiting a growing, slightly randomised amount of time between retries - prevents a mass event like a deploy or a brief network blip from causing every client to reconnect at the exact same instant and overwhelming the fleet right when it is most fragile.</p>`,
        ...
        check: {
          ...
          options: [
            ...
            'A connection registry to find that the user is on gateway B, combined with pub/sub to B\'s gateway or shard channel so the owner receives and forwards it down that specific socket',
          ...
          answer: 1,
          explain: 'The established TCP connection already remains on its owning gateway. The registry answers "which gateway owns this socket," and targeted pub/sub moves the message to that gateway for delivery; affinity is optional reconnect policy, not the lifetime-routing mechanism.'
        }
      },
      {
        id: 'rf-f3',
...
<p>Sending itself has to assume it can fail: a push provider or SMS gateway can be transiently unavailable, so a send goes through the same backoff-and-retry pattern as any external call, bounded, and protected by an idempotency key so a retried send does not double-fire the same notification. A <b>delivery receipt</b> - from the provider or client - distinguishes "accepted for sending" from "reached the device." It does not prove the user read the content; that needs a separate explicit open/read acknowledgement from the client. These signals support product measurement and decisions about falling back to another channel after delivery failure. Finally, every send checks the user's own <b>preferences</b> - a per-category, per-channel opt-in table - since even a technically correct, on-time notification is the wrong thing to send if the user has turned that category off.</p>`,
...
<p>The live viewer count is refreshed every second or two from per-gateway active-connection gauges, or from expiry-aware heartbeat buckets keyed by stable viewer id when one viewer can have several connections. A cumulative HyperLogLog is unsuitable because it cannot remove viewers who have left.</p>`,
...
Structurally, I'd route all three through a shared service that checks preferences and applies deduplication, then fan out into separate priority queues per category - security, order status, promotional - each with its own workers, so a backlog in one queue can never delay the others."`
      },
      {
        ...
        timeboxSec: 300,
        rubric: `Must-haves: (1) recognises a single server has a hard ceiling on concurrent connections (file descriptor and memory limits) so horizontal scale-out is required; (2) proposes a connection registry mapping user/connection to its owning gateway; (3) routes delivery through targeted pub/sub to the owning gateway or shard, rather than subscribing every server to every per-user channel; (4) explains that an established TCP connection already stays on its owner, while affinity is optional reconnect or related-request policy; (5) mentions heartbeats and expiring registry leases for dead connections. Common mistakes: proposing only "add more servers" with no cross-server delivery; claiming sticky sessions are required to keep an established socket on its server; broadcasting every message to every gateway; treating a consumer group as a substitute for socket ownership lookup. {{HONESTY}}`,
...
That creates a problem 10,000 connections on one server never had: a message handled by server A may need to reach a socket owned by gateway B. I would add a connection registry mapping user and connection ids to the owning gateway. The sender looks up B, then publishes to B's gateway or shard channel; only that owner receives the event and forwards it down the local socket. The registry record is a renewable lease so it expires if B dies.

An established TCP connection already stays on the server that accepted it; load-balancer affinity is optional for reconnects or related HTTP requests, not a requirement for the socket's lifetime. I would still want disciplined heartbeats because a leaked phantom connection that was a minor annoyance at 10,000 connections becomes a real problem across 2 million.
