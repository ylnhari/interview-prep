...
<li><b>Delta Lake</b> - a storage format adding ACID (atomicity, consistency, isolation, durability - the guarantees that make a transaction commit as a safe, all-or-nothing unit) transactions and version history ("time travel") on top of files in a data lake, most often used with Spark.</li>
...
<li><b>dvc.yaml, pipeline stage</b> - a file declaring a data or training pipeline as a DAG (directed acyclic graph - steps connected by dependency, with no path that loops back on itself) of named stages, each with its inputs, outputs, and the command that produces them.</li>
...
<li><b>GitOps</b> (deploying by committing the desired state to Git rather than running deploy commands by hand)<b>, ArgoCD, Flux</b> - the pattern where Git holds the desired state of a deployed system and a controller continuously reconciles the running system to match it; ArgoCD and Flux are the two most widely used controllers implementing it for Kubernetes.</li>
...
<p>Three metric types cover almost everything worth tracking. A <b>counter</b> only ever goes up - total requests served, total prediction errors - and you read it by looking at its rate of change over a window, not its raw value. A <b>gauge</b> can go up or down, like the current queue depth or the number of loaded model replicas. A <b>histogram</b> buckets observed values - request latency is the standard example - so a percentile like p99 (the 99th percentile - the value below which 99% of observed requests fall, used to describe the slowest few without being thrown off by a single outlier) can be computed later from the buckets, rather than needing to store every individual value.</p>
