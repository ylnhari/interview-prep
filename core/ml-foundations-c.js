    ...
    learn: [
      {
        ...
        part: 'field',
        title: 'Key terms',
        body: `<p>The terms move from the basic units and training process to stability, architectures, and common uses.</p>
<ul>
...
<li><b>Forward pass</b> - computing a network's output for a given input, layer by layer, from input to output. It is all that runs when a trained model serves a prediction; training adds a backward pass after it.</li>
<li><b>Loss function</b> - <a href="#ml-math-essentials/mfe-f5">taught in Maths for machine learning</a>: the single number a training run tries to make small, measuring how wrong the forward pass's output was.</li>
...
<li><b>Epoch, batch</b> - one epoch is one full pass through the training data; a batch (or mini-batch) is the small slice of examples used to compute one gradient and take one step, commonly 32 to 512 examples.</li>
<li><b>Learning rate</b> - how big a step each weight takes in the direction opposite its gradient. Around 0.001 is a common starting value with Adam; set it ten times too high and the loss climbs or turns into not-a-number instead of falling.</li>
...
<li><b>BatchNorm, LayerNorm, RMSNorm</b> - three ways of rescaling a layer's values mid-network so training stays stable. BatchNorm normalises across the examples in a batch, so its behaviour changes with batch size; LayerNorm and RMSNorm normalise within one example, which is why transformers use them.</li>
...
<li><b>Weight decay</b> - shrinking every weight slightly on every step, independent of the gradient, to discourage very large weights. A value around 0.01 is typical when training a transformer.</li>
...
<li><b>Dropout</b> - <a href="#data-and-generalization/dag-f7">taught with the other regularisers</a>: a fraction of a layer's outputs is randomly zeroed during training, so the network cannot rely on any one unit.</li>
<li><b>Early stopping</b> - <a href="#data-and-generalization/dag-f7">taught in Data, features and generalisation</a>: training stops once held-out performance stops improving.</li>
<li><b>Data augmentation</b> - <a href="#data-and-generalization/dag-f7">taught with the other regularisers</a>: new, label-preserving training examples made by transforming existing ones, such as flipping or cropping an image.</li>
...
<li><b>RNN (recurrent neural network), LSTM (long short-term memory), GRU (gated recurrent unit)</b> - three network designs for a sequence, each carrying a hidden state forward from one step to the next; LSTM and GRU add gates that decide what to keep or forget, to fight the vanishing-gradient problem these networks have over long sequences.</li>
...
<li><b>Encoder-decoder, decoder-only</b> - two transformer layouts. An encoder-decoder has a bidirectional encoder feeding a generating decoder; a decoder-only model is a single stack where every position can only see the ones before it. The original translation transformer was an encoder-decoder; the large language models behind today's chat assistants are decoder-only.</li>
...
<li><b>GAN (generative adversarial network), VAE (variational autoencoder), diffusion model</b> - three families of generative model that learn to produce new examples resembling the training data: a GAN trains a generator against a discriminator that tries to spot the fakes, a VAE learns a distribution it can sample from, and a diffusion model learns to undo added noise one step at a time.</li>
...
<p>Without any non-linearity, none of this matters: a matrix multiply followed by another matrix multiply is still just one matrix multiply. A ten-layer linear network computes exactly the same family of functions as a one-layer linear network. The non-linearity is the only thing that lets depth add real computing power.</p>`,
      ...
      },
      {
        ...
        body: `<p>The <b>forward pass</b> is repeatedly applying the neuron equation, layer after layer, until the input has become the network's output. For classification, the final layer usually applies <a href="#ml-math-essentials/mfe-f6">softmax</a>, turning raw scores into probabilities that add to one; for regression, the final layer is usually left as raw numbers.</p>
<p>The <b>loss function</b> turns the output into a single number: how wrong was this prediction. For regression, the standard choice is <b>mean squared error</b>: <code>L = (y_hat - y)^2</code>, averaged over a batch. For classification, the standard choice is <a href="#ml-math-essentials/mfe-f5">cross-entropy</a>: <code>L = -log(y_hat_c)</code>, where <code>y_hat_c</code> is the probability the model assigned to the correct class. A confident, correct prediction gives a loss near zero; a confident, wrong prediction gives a large loss, because <code>-log</code> grows without bound as its argument approaches zero.</p>
...
<p>Normalisation is the same problem solved mid-network instead of only at the start. <b>BatchNorm</b> takes one feature at a time and rescales it to zero mean and unit variance <i>across the examples in a batch</i>, then lets the network learn a new scale and shift. It works well for convolutional networks with reasonably large batches, but its statistics get noisy at a batch size of one, and it needs different behaviour at training time (using the batch's own statistics) versus inference time (using a running average collected during training). <b>LayerNorm</b> normalises the other way: for one example, across its own features, independent of batch size entirely - which is why it is the default inside transformers and RNNs (plural of RNN), where batch size varies and sequences of different lengths are processed. <b>RMSNorm</b> is LayerNorm with the mean-centring step dropped: it only rescales by the root-mean-square of the values, cheaper to compute, and is what most current large language models use.</p>`,
...
<p>The learning rate itself usually follows a schedule. <b>Warm-up</b> ramps it up from near zero over the first steps, because Adam's moment estimates are unreliable when they have only seen a handful of gradients, and a large step on bad estimates can permanently damage the weights. After warm-up, <b>cosine decay</b> brings the learning rate back down smoothly along a cosine curve toward a small floor, so training makes large, fast progress early and small, careful adjustments late. <b>Weight decay</b> shrinks every weight a little on every step regardless of its gradient, discouraging weights from growing large purely because that reduces training loss without generalising. <b>Gradient clipping</b> caps the size of the gradient before it is applied, so one unusually bad batch cannot take a step large enough to destabilise the whole run.</p>`,
      ...
      },
      {
        ...
        body: `<p><a href="#data-and-generalization/dag-f7">Dropout, early stopping and data augmentation</a> are taught whole in the foundations chapter, and they apply to a network exactly as described there. Three things about them are specific to networks and worth having ready. <b>Dropout</b> belongs on the wide fully connected layers, and is normally left off convolutional layers, which already share weights and have far fewer parameters to overfit with; a network using BatchNorm usually needs less of it, because the batch statistics are themselves a source of noise. <b>Early stopping</b> on a network means keeping the checkpoint that scored best, not the last one, since a network's validation loss commonly wanders up and down before it settles. <b>Data augmentation</b> is where most of the gain actually comes from in vision and audio, because it attacks the real problem - not enough varied data - rather than only constraining the model after the fact: flipping, cropping and colour jitter for images, back-translation or synonym swaps for text, small time shifts or added noise for a time series.</p>
...
<p>Transformers have an important training advantage over recurrent networks: attention across sequence positions can be computed in parallel with matrix operations, while a recurrent network must process positions in order. Attention also gives positions a direct path to other positions, rather than carrying information through every intervening recurrent step. Large transformer models have shown predictable improvements as training data and compute increase in some studied regimes; this empirical scaling is not a guarantee for every task or a result explained by parallelism alone.</p>`,
        deeper: null,
        check: {
    ...
    learn: [
      {
        ...
        part: 'field',
        title: 'Key terms',
        body: `<p>The terms follow a model's path from repository and data through training, deployment, and monitoring.</p>
<ul>
...
<li><b>Delta Lake</b> - a storage format adding <a href="#sql-databases/sql-f4">ACID</a> (atomicity, consistency, isolation, durability) transactions and version history ("time travel") on top of files in a data lake, most often used with Spark.</li>
...
<li><b>dvc.yaml, pipeline stage</b> - a file declaring a data or training pipeline as a <a href="#queues-and-streams/qs-f7">DAG</a> (directed acyclic graph) of named stages, each with its inputs, outputs, and the command that produces them.</li>
...
<li><b>Model registry</b> - a versioned catalogue of trained models and their run, metrics, and artifacts. A deployment can refer to a specific version or a named alias; a GitOps deployment commonly records the chosen version in its Git manifest.</li>
<li><b>Promotion stage</b> - older MLflow releases used fixed labels such as staging and production; MLflow deprecated these stages in version 2.9 in favor of flexible aliases and tags on model versions.</li>
...
<li><b>GitOps, ArgoCD, Flux</b> - Git holds the desired state of a deployed system, and a controller such as ArgoCD or Flux reconciles the running system to match the Git manifest. The controller does not automatically watch a model registry stage.</li>
...
<li><b>Resource requests and limits</b> - a request is what a Pod is guaranteed and scheduled against; a limit is the hard ceiling it cannot exceed. A Pod that goes past its memory limit is killed and restarted; one that goes past its CPU limit is throttled instead.</li>
<li><b>GPU node pool</b> - a group of cluster nodes with GPUs attached. A node taint repels Pods without a matching toleration; a GPU workload normally needs both that toleration and a GPU resource request, plus a selector or affinity rule when placement must be restricted.</li>
<li><b>Prometheus</b> - a monitoring system that pulls numeric metrics from services on a schedule - typically scraping each service's <code>/metrics</code> endpoint every 15 seconds - and stores them as time series.</li>
<li><b>Counter, gauge, histogram, summary</b> - Prometheus metric types: a counter only increases, a gauge can rise or fall, and a histogram groups observations into buckets so quantiles can be calculated and aggregated later. A summary calculates configured quantiles in the instrumenting process; those quantiles cannot be aggregated across processes.</li>
<li><b>PromQL, Alertmanager, Grafana</b> - PromQL is Prometheus's query language, where an expression such as <code>rate(http_requests_total[5m])</code> turns a counter into a per-second rate; Alertmanager turns a PromQL condition into a routed notification; Grafana is the usual tool for turning those metrics into dashboards.</li>
</ul>`,
...
<p>The <b>model registry</b> sits on top of runs: it is where a specific trained model, produced by a specific run, gets registered as a named, versioned entity, separate from the experiment that produced it. A deployment can load an explicitly pinned version or use a named alias that points to a version. An alias can be reassigned after approval, making rollout and rollback traceable without relying on a raw file path. For a GitOps deployment, the Git manifest should record the chosen version; a controller does not automatically discover changes to a registry alias.</p>
<p>Older MLflow workflows used fixed stages such as staging and production, but MLflow deprecated those stages in version 2.9 and recommends aliases and tags for model versions. Other registries may use their own lifecycle labels. In either case, make clear how the release decision changes the version that the serving system actually loads.</p>`,
        deeper: null,
        check: {
...
<p>Three more things belong in the endpoint itself. <a href="#serving-and-scale/ss-f3">Batching</a> collects several incoming requests over a short window and runs them through the model together; in FastAPI this means a background task holding a queue, not a per-request loop. A <a href="#apis-and-communication/apc-f5">health check</a> splits into readiness (should this replica currently receive traffic) and liveness (should this process be restarted), and a model server's readiness check must return false until the weights are actually loaded, which can take a minute. A <b>timeout</b> should wrap the model inference call specifically, not the whole request handler, so a slow-but-not-dead model still fails a request cleanly within a bounded time instead of holding a connection open indefinitely.</p>`,
        deeper: null,
        check: {
...
<p><b>Promoting a model</b> is the step that turns a registered candidate into what actually serves traffic, and should be deliberate: automated checks compare it with the model already in production using the release metrics and thresholds, followed by approval before release. A registry alias or lifecycle label can record that decision, but it changes production only if the serving system is configured to read it.</p>
<p>Deployment itself is where <a href="#ml-lifecycle-platform/mlp-f4">GitOps</a> takes over from CI. For a GitOps deployment, the pipeline's last step is a reviewed commit that updates the desired model version and image tag in a Git manifest. A controller such as ArgoCD or Flux then reconciles the cluster to that manifest; it does not automatically watch a model registry stage or alias. If a system resolves a registry alias instead of pinning a version in Git, that integration must be configured explicitly. This keeps the release reference reviewable in Git and means CI does not need credentials for the production cluster.</p>`,
        deeper: null,
        check: {
...
<p><b>Resource requests and limits</b> matter for an ML workload because a standard request for <code>nvidia.com/gpu</code> normally reserves an exclusive GPU for that Pod. A cluster can also be configured to share GPU access through time-slicing, or to divide supported GPUs into isolated instances with Multi-Instance GPU (MIG). These modes have different isolation and capacity guarantees, so check how the device plugin exposes them. A <b>GPU node pool</b> can use a taint to repel Pods without a matching toleration; a toleration only permits placement, so pair it with a GPU resource request and a selector or affinity rule when needed.</p>
<p>The <a href="#serving-and-scale/ss-f2">Horizontal Pod Autoscaler (HPA)</a> changes the replica count automatically, based on an observed metric. What is specific to an ML service is which metric: CPU usage is the Kubernetes default and is close to useless for a GPU server, whose CPU sits idle while the GPU saturates, so scale instead on queue depth or GPU utilisation, fed to the HPA through a metrics adapter reading from Prometheus.</p>`,
        deeper: null,
        check: {
          ...
          options: [
            ...
            'A standard nvidia.com/gpu request normally gives the Pod exclusive access to a whole GPU; a cluster can instead be configured for time-slicing or supported MIG instances, each with different sharing and isolation guarantees',
            'CPU and memory requests automatically include GPU access',
            'nvidia.com/gpu is a label, not a schedulable resource'
          ],
          answer: 1,
          explain: 'A standard GPU resource request is exclusive by default, but an administrator can configure GPU time-slicing or MIG. The request and its guarantees therefore depend on how the device plugin advertises the hardware.'
        }
      },
      {
        id: 'mot-f8',
...
<p>Prometheus has four core metric types. A <b>counter</b> only increases - total requests served or prediction errors - and you read it by looking at its rate of change over a window, not its raw value. A <b>gauge</b> can go up or down, like the current queue depth or number of loaded model replicas. A <b>histogram</b> buckets observations such as request latency, so quantiles can be calculated from the buckets and aggregated across instances. A <b>summary</b> calculates configured quantiles in the instrumenting process; those quantiles cannot be aggregated across instances. Histograms are generally the better choice when a dashboard needs fleet-wide latency quantiles.</p>
...
<p>An ML service needs one more layer beyond the request-count-and-latency metrics any service exposes: <b>model-quality metrics</b>, tracking what a system health dashboard cannot see on its own - the distribution of the model's own predictions, a <a href="#reliability-ops/ro-f3">drift</a> score against a reference window, and, once labels catch up, actual accuracy. A service can sit at perfect uptime and latency while quietly making worse and worse decisions, which is why these are kept on a <a href="#reliability-ops/ro-f1">second dashboard</a>, read as a separate question from the first.</p>`,
        deeper: null,
        check: {
...
<p>Once approved, the actual mechanism is a change to a manifest in Git: the model version or image tag the Kubernetes Deployment references gets updated in a pull request, itself reviewed. A GitOps controller like ArgoCD notices that change and reconciles the cluster to match it - it is not the pipeline pushing to the cluster directly. Rather than switching all traffic at once, I would want the rollout itself to be progressive - a small percentage of traffic first, checked against the release metrics, before it ramps to everyone. Once it is serving, Prometheus and Grafana pick up the story: request-level health as always, plus the model-quality metrics specifically, because a clean rollout on latency and error rate says nothing yet about whether the new model's decisions are actually good.</p>`
      },
      {
        ...
        prompt: 'Extend the single-prediction endpoint from this chapter into one that accepts a batch of up to 32 requests in one call. Describe the Pydantic models and endpoint, and explain what happens to the whole batch if any item is invalid.',
        timeboxSec: 300,
        rubric: `Must-haves: (1) a request model wrapping a list of typed prediction requests with a maximum length of 32 declared on the model; (2) explains that Pydantic validates every nested item before the endpoint runs, so any malformed item rejects the entire request with 422 and no predictions run; (3) defines the all-or-nothing response contract for a valid batch and gives a reason, such as simpler retries and avoiding mixed results; (4) distinguishes request-level batching from internal GPU micro-batching, which FastAPI does not provide automatically; (5) notes that the declared length constraint also rejects empty or oversized batches before the function runs. Common mistakes: claiming to return per-item validation failures while using a typed nested request model; continuing inference for valid items after FastAPI rejects an invalid nested item; conflating request batching with GPU micro-batching. {{HONESTY}}`,
        model: `<p>I'd declare the batch as a typed list with its bounds on the Pydantic request model: <code>class BatchRequest(BaseModel): items: list[PredictRequest] = Field(min_length=1, max_length=32)</code>. FastAPI validates both the list length and each nested <code>PredictRequest</code> before calling the endpoint. If even one item has the wrong type or is malformed, validation rejects the whole request with a 422, and the prediction function does not run. That gives the endpoint a clear all-or-nothing contract for input validation.</p>
<p>For a valid batch, I would return one prediction result per input item in the same order, with the model version included in the response. If the API must report validation errors per item instead, I would use a deliberately looser input type and validate each item explicitly; I would not promise that behavior while declaring a list of already-validated <code>PredictRequest</code> objects. Accepting up to 32 items in one API request also does not automatically make model execution efficient: internal GPU micro-batching is a separate serving behavior that has to be implemented or provided by the model server.</p>`
      },
      {
        id: 'mot-a3',
        type: 'followup',
    ...
    glossary: [
      {
        ...
        rows: [
          ...
          ['Model registry', 'A versioned catalogue of trained models, linked to their training runs and metrics. Deployments can pin a model version or use a named alias; GitOps commonly records the chosen version in a Git manifest.', 'Approving a model release and identifying exactly which version serves traffic.'],
          ...
          ['Counter, gauge, histogram, summary', "Prometheus's four core metric types: counters increase, gauges rise or fall, histograms bucket observations for aggregatable quantiles, and summaries calculate quantiles in the instrumenting process.", 'Instrumenting a FastAPI service and choosing how to track request latency.'],
