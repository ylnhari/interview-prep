...
<li><b>GitOps, ArgoCD, Flux</b> - <a href="#ml-lifecycle-platform/mlp-f4">taught in The ML platform, end to end</a>: Git holds the desired state of a deployed system and a controller reconciles the running system to match it. ArgoCD and Flux are the two most widely used such controllers for Kubernetes.</li>
...
<p>Deployment itself is where <a href="#ml-lifecycle-platform/mlp-f4">GitOps</a> takes over from CI. The handoff is the part worth getting right: the pipeline's last step is not a deploy command but a commit, updating a manifest in Git with the new model version reference and image tag, after which the CI run is finished and a controller such as ArgoCD or Flux does the rest. That means CI never needs credentials for the production cluster, and every deployment carries an author, a timestamp, and a diff.</p>`,
        deeper: null,
        check: {
...
<p>Three metric types cover almost everything worth tracking. A <b>counter</b> only ever goes up - total requests served, total prediction errors - and you read it by looking at its rate of change over a window, not its raw value. A <b>gauge</b> can go up or down, like the current queue depth or the number of loaded model replicas. A <b>histogram</b> buckets observed values - request latency is the standard example - so a percentile like <a href="#apis-and-communication/apc-f8">p99</a> (the 99th percentile) can be computed later from the buckets, rather than needing to store every individual value.</p>
