...
<p>Logistic regression is one member of a larger family, <b>generalised linear models</b>: instead of predicting the target directly, a GLM predicts a transformed version of its average through a link function, using the same linear score underneath. Poisson regression is the GLM for counts (number of claims, number of clicks): it uses a log link, so <code>log(expected count) = z</code>, guaranteeing a positive prediction, and it assumes the outcome's variance equals its mean; when real data is noisier than that (over-dispersion), a negative binomial model is used instead.</p>`,
      ...
      },
      {
        ...
        body: `<p>A classifier is <b>calibrated</b> if, among all the rows it assigns a probability of 0.7, roughly 70% actually belong to the positive class. Accuracy and <a href="#evaluation-and-selection/es-f4">AUC</a> (area under the ROC (receiver operating characteristic) curve) only care about whether positive rows score higher than negative ones, not whether the actual numbers mean anything -- so a model can rank perfectly and still be badly calibrated. This matters whenever a downstream decision multiplies the probability by something, such as an expected-value calculation.</p>
...
<p>Boosted trees and neural networks can both be miscalibrated, for different reasons. A tree ensemble's leaf averages can move toward extreme probabilities as boosting rounds keep correcting predictions toward the training labels. A neural network trained with cross-entropy can also produce overconfident probabilities: cross-entropy rewards correct classification and probability fit together, but minimizing it does not guarantee that predicted probabilities match observed frequencies on new data. Check calibration on held-out data rather than inferring it from accuracy alone.</p>`,
...
<p>The hyperparameters that matter most: the number of trees, the learning rate (these two trade off directly), the tree depth or number of leaves, the fraction of rows and columns sampled per tree (subsample, colsample), a minimum number of samples required in a leaf, and the L1/L2 regularisation strength. Gradient boosting wins so often on tabular data because it handles messy, non-smooth, and interacting features without any scaling, copes natively with missing values, and its additive sequential correction can approximate almost any function to arbitrary precision given enough shallow trees -- exactly the setting where a single linear model is too rigid and a plain neural network has no strong inductive bias to exploit.</p>`,
        deeper: `<p><a href="#data-and-generalization/dag-f7">Early stopping</a> is what makes the learning-rate/number-of-trees trade-off practical: train with a small learning rate and a large maximum number of trees, and let the validation loss decide where to stop instead of guessing the number of rounds up front.</p>`,
        check: {
...
<p><a href="#ml-math-essentials/mfe-f2">PCA</a> reduces dimensionality by finding the directions (eigenvectors of the data's covariance matrix) along which the data varies the most, ranked by how much variance each one explains, and keeping only the top few. For <b>anomaly detection</b>: an <b>isolation forest</b> builds many trees of random feature/threshold splits and scores a point by how few splits it took to isolate it alone -- outliers are "few and different," so they isolate fast; an <a href="#deep-learning-essentials/dle-f9">autoencoder</a> scores a point by how badly it reconstructs it, since a network that only ever learned to reconstruct normal-looking data leaves an unusually large reconstruction error on an anomaly.</p>`,
...
<p><a href="#data-and-generalization/dag-f7">Early stopping</a> is a different, complementary idea, and it removes one hyperparameter from the search entirely: instead of treating the number of training rounds as a dimension to search over, set a generous maximum and let the validation loss decide where each trial stops. Every trial then costs only as long as it was still improving, which is often the single biggest saving available in a search over a boosted-tree or neural model.</p>
