...
<li><b>A30.</b> A shared cache that every app server talks to sidesteps consistency by having only one copy of the cached value, at the cost of a network hop. A local, in-process cache on each server is faster but now has N independent copies that can disagree, so invalidation must be broadcast to every instance, typically via pub/sub. A common pattern uses both: a shared cache as the source of cached truth, with a short-TTL local cache in front for the hottest keys.</li>
</ul>`,
        check: null
      },
      {
...
<li><b>A50.</b> An SLI is the measured number, such as the percentage of requests under 300 milliseconds. An SLO is the internal target for that indicator, such as 99.9% under 300 milliseconds over a rolling 30 days. An SLA is the external, usually contractual, promise, often looser than the SLO to leave margin. The error budget is 100% minus the SLO, converted into allowed failure: a 99.9% SLO over 30 days permits about 43.2 minutes of downtime; once spent, the team should stop shipping new features and focus on reliability.</li>
...
<li><b>Q26.</b> How do you choose a classification threshold?</li>
        ...
        deeper: `<ul>
<li><b>A21.</b> Precision is, of items predicted positive, what fraction actually are (TP / (TP+FP)); recall is, of items that actually are positive, what fraction the model found (TP / (TP+FN)). Optimise for precision when a false positive is expensive, such as blocking a real customer's transaction as fraud; optimise for recall when a false negative is expensive, such as missing an actual cancer case. F1 is the harmonic mean, 2 x precision x recall / (precision + recall), used for a single number that penalises an extreme imbalance rather than averaging arithmetically.</li>
<li><b>A22.</b> ROC-AUC plots true positive rate against false positive rate across all thresholds and summarises the area under the curve, 0.5 random and 1.0 perfect; it is threshold-independent but can look deceptively good on imbalanced data, since false positive rate is measured against the large negative class. PR-AUC plots precision against recall instead, and because precision is directly affected by how many negatives get misclassified relative to the small number of positives, it is far more informative when positives are rare.</li>
...
<li><b>Q12.</b> What metrics matter for LLM serving (TTFT, TPOT/ITL, throughput) and how do you trade them off?</li>
...
<li><b>A24.</b> First, supervised fine-tuning on demonstration data to get a reasonable base of instruction-following. Second, training a reward model: human raters compare or rank the SFT model's outputs for the same prompt, and a separate model learns to predict which output a human would prefer, turning subjective judgment into a differentiable score. Third, reinforcement learning, commonly PPO, further tunes the model to generate outputs the reward model scores highly, while a KL-divergence penalty - KL divergence being a measure of how far one probability distribution has moved from another - keeps it from drifting so far from the original SFT model that it exploits the reward model's blind spots.</li>
...
<li><b>A29.</b> An exact nearest-neighbor search over millions of high-dimensional embeddings requires comparing the query against every stored vector, which does not scale; approximate methods trade a small, tunable accuracy loss for large speed gains. HNSW builds a multi-layer graph where each vector is a node, with sparse long-range connections in upper layers for fast coarse navigation and denser connections in lower layers for fine-grained accuracy - a search starts at the top layer, greedily moves to the closest neighbour, and descends layer by layer, giving search time that scales roughly logarithmically with the number of vectors rather than linearly.</li>
<li><b>A30.</b> Sparse retrieval, BM25, matches on exact terms weighted by rarity and concentration, excelling at exact matches - product codes, names, precise phrases - and needing no training or embedding model. Dense retrieval encodes query and documents into vectors and finds documents closest to the query's vector, capturing semantic similarity even with completely different wording. Hybrid search runs both and combines their rankings, often with reciprocal rank fusion, because the two fail in largely non-overlapping cases - a specific model number dense retrieval might paraphrase past, and a semantically related concept BM25's exact matching would completely miss.</li>
