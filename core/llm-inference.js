...
<li><b>Supervised fine-tuning (SFT)</b> - instruction tuning on curated instruction-response pairs, used as the starting model before a preference-alignment stage.</li>
<li><b>Policy</b> - in reinforcement learning, the model being trained: a rule that maps its current input to a distribution over its next response.</li>
...
<li><b>DPO (direct preference optimisation)</b> - training directly on pairs of preferred and dispreferred responses with a single loss, without a separate reward model or a reinforcement-learning loop.</li>
...
<li><b>Merging adapters</b> - folding a trained LoRA adapter's weights back into the base model, producing one ordinary checkpoint with no extra inference-time cost.</li>
      ...
      },
      {
        ...
        viz: 'rag-pipeline',
        body: `<p>Start with the cheapest option: a better prompt has no training run and can be changed in seconds, so it is the first thing to test for a problem about the model's <i>behaviour</i> - its format, tone, or how carefully it follows an instruction. Its tokens still have a per-request cost.</p>
<p>If the problem is that the model does not know something - a fact, a recent event, an internal document - fine-tuning is usually the wrong tool. Fine-tuning teaches a model how to respond far more reliably than it teaches new facts, and a fine-tuned fact cannot be updated the moment the source document changes; every update needs a new training run. <a href="#rag-and-agents/raa-f1">Retrieval-augmented generation</a> fixes this directly, and the answer is then grounded in a specific, current, citable source instead of whatever the model happened to memorise.</p>
...
<p>A practical ladder: prompt first; add retrieval once the answer depends on information outside the model's training data or the prompt itself; fine-tune once repeated prompting and retrieval still cannot make the model behave the way the task needs, or once cost and latency require a smaller, specialised model. The strongest production systems usually use all three together: a model fine-tuned on the task's format and voice, given a well-written prompt, and handed retrieved context for anything that can change.</p>`,
      ...
      },
      {
        ...
        viz: 'lora-adapter',
        body: `<p>Full fine-tuning updates every weight, so training must hold not just the weights but their gradients and the optimiser's own per-parameter bookkeeping. With the common Adam optimiser in mixed precision, the standard accounting is 16 bytes of state per parameter: 2 bytes for fp16 weights, 2 for fp16 gradients, and 4 bytes each for a full-precision copy of the weights and Adam's two running averages (<a href="#training-at-scale/tas-f3">worked through in Fitting a model in memory</a>). That memory bill, more than the model's usefulness, is usually what rules full fine-tuning out for anything past a few billion parameters on ordinary hardware.</p>
...
<p><b>Contamination.</b> Check that fine-tuning data does not overlap with the benchmarks or held-out set used to evaluate it - the same <a href="#data-and-generalization/dag-f5">data leakage</a> problem as training on the test set anywhere else in machine learning - and that it carries no information, such as another customer's data, that should never end up inside a model's weights.</p>`,
...
<p><b>Sequence length versus batch size and memory.</b> Activation memory grows with sequence length and batch size together, and attention's own memory use grows faster still with sequence length unless a memory-efficient implementation such as <a href="#llm-inference-engineering/lie-f5">FlashAttention</a> is used. A longer maximum sequence length therefore forces a smaller batch size to fit the same GPU memory, and the two have to be tuned as one setting, not two.</p>
<p><b>Gradient accumulation</b> and <b>mixed precision</b>, both <a href="#training-at-scale/tas-f2">taught in Training at scale and GPUs</a>, are the two settings that decide whether a fine-tune fits at all: accumulation buys a larger effective batch than memory allows, and bf16 arithmetic roughly halves the memory each value takes. For a fine-tune specifically, set the accumulation steps so that per-device batch times accumulation steps reaches the effective batch the recipe assumes, and leave mixed precision on unless a loss curve turns unstable.</p>
...
<p><b>DPO (direct preference optimisation)</b> skips the reward model and the RL loop entirely. It trains directly on the same preferred-versus-dispreferred pairs with one classification-style loss, without sampling from the model during training or running PPO's harder-to-stabilise optimisation. Under the assumptions in its derivation, that loss corresponds to a KL-constrained preference objective; it does not guarantee that any concrete DPO run and PPO run reach the same policy. It is often a practical first method because it needs one model instead of three and has fewer moving parts.</p>
...
<p><b>Versioning.</b> A checkpoint is only meaningful as the combination of a base model version, an adapter (or full fine-tune) version, and the data version it was trained on. Evaluation results are valid for that exact combination, not for the adapter on its own: moving an adapter trained against one base model onto a newer base model is not guaranteed to work, since the base's weights - and therefore what the adapter's small correction is actually correcting - have changed underneath it. Track and pin all three together, and re-evaluate after any one of them changes.</p>`,
    ...
    activities: [
      ...
      },
      {
        ...
        timeboxSec: 420,
        rubric: "Must-haves: (1) correctly describes the RLHF pipeline being replaced - a reward model trained on human preference pairs, then a PPO reinforcement-learning loop with a KL penalty back to the SFT model; (2) correctly describes DPO as training directly on the same preference pairs with a single loss, no separate reward model, no RL sampling; (3) states the practical reasons to consider it: fewer moving parts, one model to train instead of three, and no reward model to exploit; (4) does not claim DPO is a rough approximation or that it guarantees the same trained policy as PPO - it should note that its derivation targets a KL-constrained preference objective under stated assumptions; (5) does not claim more than is true by saying DPO always beats RLHF or GRPO in every setting. Common mistakes: describing DPO as skipping human feedback entirely; claiming DPO changed what data is collected rather than how it's used; treating DPO as strictly better in every case. {{HONESTY}}",
        model: "Supervised fine-tuning trains on examples of the response you want. RLHF adds a second step on top of that: collect pairs of responses to the same prompt, have humans say which one is better, and train a separate reward model to predict that preference. Then you run reinforcement learning - specifically PPO - where the language model generates a response, the reward model scores it, and the weights get nudged to make higher-scoring responses more likely, with a penalty that keeps the model from drifting too far from where it started so it can't just find a weird way to trick the reward model.\n\nThat pipeline needs three models running together - the policy, the reward model, and usually a value model - and PPO itself is finicky to tune. DPO skips the middle step: instead of training a reward model and then optimizing against it, it uses a loss directly on the same preferred-versus-dispreferred pairs. Under the assumptions in its derivation, that loss targets a KL-constrained preference objective without a separate reward model or an RL loop. That does not mean a particular DPO run is guaranteed to produce the same policy as a particular PPO run.\n\nDPO is often a useful first method because it has fewer moving parts. I would not claim it always wins; PPO and newer methods like GRPO are still used, especially at very large scale."
      }
    ],
    sayQuestion: null,
    sayItOutLoud: null,
...
<li><b>AWQ</b> (activation-aware weight quantization) - a post-training weight-quantization method that protects weights important to observed activations from quantization error.</li>
<li><b>GPTQ</b> - a post-training weight-quantization method that quantizes a layer while correcting remaining weights with approximate second-order information.</li>
...
<li><b>Pipeline parallelism</b> - <a href="#training-at-scale/tas-f6">taught in the same section</a>: each GPU holds a slice of the model's layers, in sequence.</li>
...
<li><b>Roofline model</b> - <a href="#training-at-scale/tas-f1">taught in What a GPU is good at</a>: a chart of a chip's compute ceiling and its memory-bandwidth ceiling, read to find which one caps an operation.</li>
<li><b>Service level objective (SLO)</b> - <a href="#reliability-ops/ro-f2">taught in Reliability and operations</a>: a target for a metric, such as "95 percent of requests get their first token within 500 milliseconds."</li>
<li><b>Serving runtime</b> - the software that turns a trained model into a server that accepts requests and streams tokens back, such as vLLM or TensorRT-LLM (LLM is short for large language model).</li>
      ...
      },
      {
        id: 'lie-f0',
        part: 'field',
        title: 'What a token actually is, and why two texts of the same length cost different amounts',
        body: `<p>Almost every number in language-model serving is counted in tokens: the KV cache costs so many bytes per token, a context window holds so many tokens, a provider charges so much per million tokens, a retrieved chunk is sized in tokens. A token is neither a word nor a character, so it is worth knowing exactly what one is before any of those numbers mean anything.</p>
<p>A <b>tokenizer</b> is a fixed lookup table, built once before the model is trained, that maps pieces of text to integer ids. It usually holds between 30,000 and 200,000 entries, and that set of entries is the model's <b>vocabulary</b>. Turning text into ids is called <b>tokenization</b>; turning ids back into text is <b>detokenization</b>. The model never sees characters at all - it reads a list of integers and writes a list of integers, and the tokenizer is what stands between those integers and anything a person can read.</p>
<p><b>Why the pieces are smaller than words.</b> A vocabulary of whole words cannot cover every word that will ever be typed, so an unseen word needs an unknown entry or a fallback segmentation. A vocabulary of single characters covers everything but makes sequences very long, and length is what costs money and memory. Subword tokenization is the middle: common words get one entry each, and a rare or invented word is often spelled out of several smaller pieces, so common text stays short without relying only on whole-word entries.</p>
<p><b>The three methods, by name.</b> <b>Byte-pair encoding (BPE)</b> repeatedly merges common adjacent symbols from its starting alphabet; byte-level BPE starts from bytes, while other BPE implementations may start from characters or other symbols. <b>WordPiece</b> (the method behind BERT, short for bidirectional encoder representations from transformers) chooses merges by a likelihood score rather than raw frequency: it picks the pair whose merging most increases the corpus's probability under the current vocabulary. <b>Unigram</b>, the method behind SentencePiece (a widely used open-source tokenizer library), works the other way round - it starts from a large candidate vocabulary and repeatedly drops the pieces whose removal costs the least likelihood, keeping several possible segmentations of the same string and their probabilities. Byte-level variants treat raw bytes as the alphabet, which is what guarantees that any input at all, in any script, can be encoded without an unknown token.</p>
<p><b>What follows from this in practice.</b> Token counts are not proportional to character counts. English prose runs at roughly 4 characters per token, so 1,000 tokens is about 750 words. Code runs shorter per token, because indentation, brackets and punctuation each tend to take a token of their own. Text in a language the tokenizer was not mostly trained on can take two or three times as many tokens for the same meaning, since its words fall apart into small byte-level pieces - the same sentence therefore costs more, fills the context window faster, and needs more KV cache. A leading space is usually part of the token, so " the" and "the" are different ids, which is why a prompt's exact spacing can change its token count. And because the model can only ever emit whole tokens, a constraint expressed in characters, such as "answer in under 100 characters", is not something the model can count directly.</p>`,
        deeper: `<p>Two practical consequences worth having ready. First, the only honest way to know a prompt's cost is to run the model's own tokenizer over it: the same text tokenizes to different counts under different models, so a count taken from one provider does not transfer to another. Second, tokenization is why a model is bad at character-level tasks such as counting the letters in a word or reversing a string - it never saw the letters as separate items, only the chunk they were merged into. That is a limitation of the input representation, not of the model's reasoning, and saying so is a better answer than calling it a reasoning failure.</p>`,
        check: {
          question: 'A support product translates its prompts into a language the model\'s tokenizer was not mostly trained on, and the cost per request rises sharply even though the text says the same thing. What is the most likely explanation?',
          options: [
            'The same meaning takes far more tokens, because the words break into small byte-level pieces the vocabulary does not hold as single entries',
            'The model runs a separate translation model first and charges for it',
            'Non-English text is always billed at a higher rate per token',
            'The KV cache is disabled for non-English text'
          ],
          answer: 0,
          explain: 'A tokenizer only holds whole entries for the strings common in what it was built from. Text outside that falls apart into many small pieces, so the token count, and therefore cost, context use and KV cache, all rise for the same meaning.'
        }
      },
      {
        id: 'lie-f1',
...
<p>Which of the two limits applies is read off the <a href="#training-at-scale/tas-f1">roofline model</a>. For serving, the reading is this: decode does very little arithmetic for every byte it moves - low <a href="#training-at-scale/tas-f1">arithmetic intensity</a> - so it sits under the memory-bandwidth ceiling, and no amount of extra compute power helps. Prefill's big matrix multiplications do a great deal of arithmetic per byte, so the compute ceiling is what binds instead.</p>`,
...
<p>The memory it costs, per token, is: <b>2 x layers x KV heads x head_dim x bytes per value</b>. The leading 2 accounts for storing both a key and a value; the rest is the size of one layer's key (or value) vector, repeated across every layer and every key/value head, in whatever number format the cache is stored in. In ordinary multi-head attention, KV heads equals attention heads.</p>
<p>Some models shrink this deliberately with <b>grouped-query attention</b> (or its extreme, multi-query attention): several query heads share one smaller set of key/value heads, so the KV-heads term is smaller than the number of query heads the model uses for attention itself.</p>`,
...
<p><b>Disaggregated prefill and decode</b> runs the two phases on separate pools of GPUs, since one is compute-bound and the other memory-bandwidth-bound and they compete for the same resources when forced onto one GPU; the computed KV cache is transferred to the decode pool once prefill finishes, letting each pool be sized and tuned for its own bottleneck instead of a single compromise configuration.</p>`,
      ...
      },
      {
        ...
        body: `<p>A model too large for one GPU has to be split across several. The three ways of doing that - <a href="#training-at-scale/tas-f6">tensor, pipeline and expert parallelism</a> - are taught whole in the training chapter, because the mechanism is identical whether the model is being trained or served. What is specific to serving is which one to pick and what it costs a request.</p>
<p><b>Tensor parallelism</b> communicates on every layer, so it needs the fast links inside one server and it adds a fixed communication cost to every single decode step - the step that was already the latency bottleneck. <b>Pipeline parallelism</b> communicates only at stage boundaries, so it tolerates a slower link and can cross machines, but a request now has to travel through every stage in turn, which adds to time to first token and leaves stages idle unless enough requests are being processed at once to keep the pipeline full. The rule of thumb: use tensor parallelism inside a machine, pipeline parallelism between machines, and only as much of either as the model actually requires, since both trade latency for the ability to hold the model at all.</p>
<p><b>Expert parallelism</b> applies to a <b>mixture-of-experts (MoE)</b> model, where a small <b>gating network</b> sends each token to only the top few of many expert sub-networks. Serving one raises a problem training does not have to answer per request: the experts a batch needs are only known once the gating network has run, so the all-to-all step that ships each token to the GPU holding its expert sits in the middle of every decode step, and a batch whose tokens all route to the same few experts leaves the other GPUs idle while those few become the bottleneck.</p>
    ...
    activities: [
      {
        ...
        prompt: "We're serving a 13-billion-parameter model with ordinary multi-head attention: 40 layers, 40 attention heads and therefore 40 KV heads, head dimension 128, at 8,000 tokens of context, in fp16. Treat the GPU's 80 GB capacity as decimal GB. Walk me through how much KV cache memory one sequence needs, and how many concurrent sequences fit alongside the model weights.",
        timeboxSec: 420,
        rubric: 'Must-haves: (1) states the formula, bytes per token = 2 x layers x KV heads x head_dim x bytes per value, and uses the stated 40 KV heads; (2) correctly computes bytes per token: 2 x 40 x 40 x 128 x 2 = 819,200 bytes, or 819.2 kB; (3) correctly scales to context length: 819,200 x 8,000 = 6,553,600,000 bytes = about 6.55 decimal GB per sequence; (4) correctly computes model weight memory at fp16: 13 billion x 2 bytes = 26 decimal GB; (5) correctly subtracts to find memory left for cache: 80 - 26 = 54 GB, and divides by the per-sequence cache size to get roughly 8 concurrent sequences; (6) notes this ignores activation memory and other overhead, so the real number is somewhat lower; bonus: notes that grouped-query attention, using fewer KV heads than query heads, would shrink this directly. Common mistakes: forgetting the factor of 2 for key and value; using total attention heads when the model uses grouped-query attention with fewer KV heads; mixing GB and GiB; forgetting to subtract the weight memory before dividing. {{HONESTY}}',
        model: "Using the stated multi-head-attention assumption, KV heads equals attention heads. The formula is 2 times layers times KV heads times head dimension times bytes per value. Here that's 2 x 40 x 40 x 128 x 2, which comes out to 819,200 bytes, or 819.2 kB per token.\n\nAt 8,000 tokens of context, one sequence's cache is 819,200 x 8,000 = 6,553,600,000 bytes, or about 6.55 decimal GB. I am keeping decimal GB throughout because the problem states an 80 GB GPU.\n\nThe model's own weights at fp16 are 13 billion parameters times 2 bytes, so 26 GB. That leaves 80 minus 26, or 54 GB, for the KV cache. Dividing 54 GB by about 6.55 GB per sequence gives about 8.2, so I would plan for at most 8 concurrent sequences before reserving room for activation memory and other overhead. If this model used grouped-query attention with, say, 8 KV heads instead of 40, the cache size would shrink by that same factor; I would check the actual config rather than assume the two head counts match."
      },
      {
        id: 'lie-a2',
        type: 'design',
