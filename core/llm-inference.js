...
<p><b>Why the pieces are smaller than words.</b> A vocabulary of whole words cannot cover every word that will ever be typed, so any unseen word becomes a single "unknown" entry and its meaning is lost. A vocabulary of single characters covers everything but makes sequences very long, and length is what costs money and memory. Subword tokenization is the middle: common words get one entry each, and a rare or invented word is spelled out of several smaller pieces that are all in the vocabulary, so nothing is ever unrepresentable and common text still stays short.</p>
<p><b>The three methods, by name.</b> <b>Byte-pair encoding (BPE)</b> starts from single bytes and repeatedly merges whichever adjacent pair occurs most often in the training corpus, recording each merge; the recorded merge list, applied in order, is the tokenizer. <b>WordPiece</b> (the method behind BERT, short for bidirectional encoder representations from transformers) merges by a likelihood score rather than raw frequency: it picks the pair whose merging most increases the corpus's probability under the current vocabulary. <b>Unigram</b>, the method behind SentencePiece (a widely used open-source tokenizer library), works the other way round - it starts from a large candidate vocabulary and repeatedly drops the pieces whose removal costs the least likelihood, keeping several possible segmentations of the same string and their probabilities. Byte-level variants of all three treat the raw bytes as the alphabet, which is what guarantees that any input at all, in any script, can be encoded without an unknown token.</p>
