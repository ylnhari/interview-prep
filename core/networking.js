...
<p><b>Status codes.</b> The first digit gives the broad class. 2xx worked (200 fine, 201 created, 204 nothing to send back). 3xx means look elsewhere (301 permanently moved, 304 your cached copy is still good). 4xx means the request cannot succeed as sent (400 malformed, 401 not authenticated, 403 authenticated but not allowed, 404 no such thing, 409 conflicts with current state, 429 too many requests). 5xx means the server or an upstream dependency failed (500 it broke, 502 a server upstream gave a bad answer, 503 temporarily unable, 504 something upstream timed out). The class alone is not a retry policy. Most validation and authentication 4xx responses need the request changed, while a 408 timeout or 429 rate limit can be retried when the operation is safe and the response's retry guidance allows it. A 5xx can be transient too, but it still needs a safe operation, a timeout, backoff and a retry limit rather than an automatic repeat.</p>
...
<p><b>Why DNS is a poor failover mechanism.</b> It is tempting to fail over by changing the address a name points at. In practice the change reaches clients slowly and unevenly: caches hold the old answer for the whole time to live, some resolvers round short values up, and some client libraries resolve a name once at start-up and never again. If you need failover in seconds, do it somewhere you control - an address that stays the same while the traffic behind it moves, or a load balancer that changes which servers it uses. Keep DNS for changes you can wait out.</p>`,
        deeper: `<p>The lookup is a good example of why UDP exists. A DNS query and its answer are usually one small packet each, so a TCP handshake would double the cost of the exchange to gain a guarantee that a simple retry provides just as well. Encrypted variants that run the same queries over TLS or HTTPS accept that extra cost in exchange for hiding which names you look up, which is a privacy decision rather than a performance one. And the reason a resolver can answer at all without asking anyone is the cache: with a 300 second time to live, at most one cold lookup per shared resolver cache pays the full walk during that window, assuming the entry remains cached. A different resolver or an evicted entry can still need its own walk.</p>`,
        check: {
