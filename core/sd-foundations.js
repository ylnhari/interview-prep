...
<p>A useful rule of thumb: public and browser-facing, reach for REST; two backend services you own calling each other with a low latency budget, reach for gRPC; one endpoint feeding several very different client views of the same data, reach for GraphQL.</p>`,
      ...
      },
      {
        ...
        body: `<p>Everything in this section sits at the <b>transport layer</b>, layer 4, where bytes move between an address and a port; HTTP sits one level up at the <b>application layer</b>, layer 7, where the message itself is visible (see <a href="#networking-basics/nb-layers">The network layers, all seven of them</a>). Keeping those two apart is what makes the load-balancing choice later in this chapter a real choice rather than a pair of labels.</p>
<p><b>TCP</b> establishes a connection with a handshake, then guarantees the receiver gets every byte, in order, retransmitting anything lost. That guarantee is exactly what most application protocols, including HTTP, are built on. The cost is that a single lost packet can block everything queued behind it on that connection until it is retransmitted - head-of-line blocking at the <a href="#networking-basics/nb-layers">transport layer</a>.</p>
...
<p><b>HTTP/2</b> multiplexes many logical request streams over a single TCP connection, which removes the old need for several parallel connections per host and fixes head-of-line blocking at the HTTP layer. It does not fix TCP's own head-of-line blocking: one lost TCP packet still stalls every multiplexed stream sharing that connection, since TCP has no idea they are logically independent. HTTP/3 is built on QUIC (a transport protocol that runs on top of UDP and re-implements TCP's reliability itself, one stream at a time); it moves multiplexing below that layer specifically to remove this remaining blocking - worth knowing the name exists, without needing the implementation detail.</p>
...
<p><b>Hedged requests</b> are one direct mitigation: send the request to one replica, and if the response has not come back after a short wait (often set around the typical, p50-ish latency - p50 is the median, the point half of all requests are faster than), send a second, duplicate request to a different replica - then use whichever response comes back first and discard the other. This trades a modest amount of extra load for a meaningfully better tail, and it only works when the two attempts are genuinely independent (different replicas, ideally not sharing whatever caused the first one to be slow).</p>
...
I'd lean toward <a href="#networking-basics/nb-layers">layer 7</a>, since establishing a WebSocket starts as a normal HTTP request with an upgrade header, and I want to be able to read that request - check auth, maybe route based on a user id - before deciding which replica gets the long-lived connection. <a href="#networking-basics/nb-layers">Layer 4</a> can't see any of that.
