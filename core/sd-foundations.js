...
<li><b>CAP theorem.</b> Its name spells out the three properties it is about: Consistency, Availability, Partition tolerance. Under a network partition, a distributed data store must choose between staying consistent and staying available. It says nothing about behaviour when there is no partition.</li>
<li><b>PACELC.</b> An extension of CAP; its letters spell out the choice as one sentence - Partition, Availability, Consistency, Else, Latency, Consistency: if there is a Partition, a system chooses between Availability and Consistency; Else, when there is no partition, it still trades Latency against Consistency, because waiting for more replicas to agree always costs time.</li>
</ul>`,
      ...
      },
      {
        ...
        body: `<p>A prompt such as "design a photo-sharing service" is not a specification; it is an invitation to build one. The first move is to ask a short, structured set of questions and turn the answers into a scope you both agree on, before any box is drawn.</p>
...
<li><b>Latency against consistency.</b> PACELC's addition to CAP: even with no partition at all, coordinating replicas before answering still costs time. How many replicas acknowledge a write mainly changes its durability and availability; it does not, by itself, define what a later read is guaranteed to see. Strong read consistency also needs a protocol such as leader-routed reads or intersecting read and write quorums with version/conflict rules (see <a href="#distributed-coordination/dc-3">Replication and quorums</a>). A nearest-replica read is often faster but can be stale. This trade-off exists on ordinary requests, not just during a rare partition.</li>
...
<ul>
<li>L1 cache reference (L1 is the small, fastest memory built directly into a processor core): about 0.5 nanoseconds. Main memory reference: about 100 nanoseconds - roughly 200 times slower than L1.</li>
...
<p>What matters instead is the latency budget: if the caller needs a decision within a 300 ms p99 budget, a <b>feature lookup</b> (reading the input values used for this checkout decision) costs a same-datacentre round trip (about 500 microseconds) plus a few SSD reads for less-common values. The <b>model call</b> (running the learned scoring function) and the <b>policy check</b> (applying fixed business rules to that score) have to fit in what remains. These are illustrative assumptions, not fixed hardware promises. Contrast the two examples: the photo service is a storage-and-bandwidth problem; the fraud API is a latency-budget problem with a modest QPS. Estimating both the same way - traffic, then storage, then bandwidth or latency - shows which one actually constrains the design.</p>`,
        deeper: `<p>Notice that neither example needed a precise number, only a defensible one. If an interviewer challenges an assumption - "would you really see a 200:1 read:write ratio?" - explain the workload behind it: for a feed, users may browse many posts for each one they publish. Treat 200:1 as an estimate to validate, not a fact to defend to the decimal place.</p>`,
        check: {
      ...
      },
      {
        ...
        viz: 'scale-up-vs-out',
        body: `<p><b>Vertical scaling</b> means moving to a bigger machine: more CPU, more memory, a faster disk. It is simple - little or nothing about the software has to change, and one authoritative copy can make strong consistency much easier to provide. The cost is a hard ceiling: the largest machine you can buy is still finite, cost tends to rise faster than capacity as you approach that ceiling, and the machine is a single point of failure unless another copy can take over.</p>
<p><b>Horizontal scaling</b> means adding more machines that each carry a share of the load. It removes the single-machine ceiling, not every ceiling: coordination, network, partition-count and cost limits still appear. Stateless services scale out most simply because any replica can answer any request. Stateful systems can also scale horizontally, but their data has to be partitioned or replicated and coordinated. In either case the design adds real costs: traffic distribution, partial failures and consistency questions that a single authoritative copy did not have.</p>
...
<p><b>The usual order of escalation</b> for a service that is slow or struggling under load: measure what is actually slow before changing anything; fix an inefficient query or a missing index; fix an N+1 call pattern (one query to fetch a list, then one more query per item in that list, so N+1 queries run where one or two would do); add a cache in front of the hot read path; add a read replica; and only then consider partitioning, sharding, or splitting a service. Each step is cheaper and more reversible than the one after it, which is exactly why it comes first.</p>
<p><b>Premature scaling</b> is the recognised failure of skipping that order: reaching for a distributed cache, a message queue, or a microservice split to solve a problem that a single fixed query or a bigger machine would have solved for a fraction of the cost. Each extra dependency adds a failure mode: a cache outage can turn every request into a database read, and a stalled queue can delay work until its callers time out. Engineers then have to diagnose and respond to failures across more services, even though the original bottleneck could have been fixed more simply.</p>
...
First thing I would build: the redirect path with its cache, since that is where nearly all the traffic and nearly all the risk actually is."`
      },
      {
        id: 'rac-a2',
        type: 'formulate',
        title: 'Turn "it just needs to be reliable and fast" into measurable requirements',
        prompt: `An interviewer says: "This service just needs to be reliable and fast, nothing fancy." Turn that sentence into three or four concrete, measurable non-functional requirements you would actually design against. They may be numeric targets or testable guarantees.`,
        timeboxSec: 300,
        rubric: `Must-haves: (1) does not accept the vague statement as-is, and explicitly restates it as a request for measurable targets; (2) proposes a specific availability target (e.g. 99.9%) rather than "as high as possible"; (3) proposes a specific latency target tied to a percentile (e.g. p99 under 300ms) rather than "fast"; (4) proposes a testable consistency or staleness guarantee appropriate to the guessed domain (e.g. the writer sees the change immediately and other feeds converge within two seconds); (5) proposes a testable durability guarantee for acknowledged work, such as surviving one availability-zone loss without losing an accepted order; (6) asks one clarifying question back rather than inventing targets with total confidence, since "reliable and fast" for a chat app and for a payments API mean very different commitments. Common mistakes: proposing 99.999% and sub-10ms latency by default regardless of the domain; calling a semantic guarantee a number; leaving "reliable" and "fast" unmeasurable in the final answer. {{HONESTY}}`,
...
Consistency: the customer who places an order must see it on their next read, and any non-critical replica-backed order history must converge within two seconds. That gives both the session guarantee and a staleness bound I can test.

Durability: once an order is acknowledged, losing any one availability zone must not lose it. That is separate from availability: the service can be briefly unreachable while the accepted order remains durable.

That turns 'reliable and fast' into four measurable commitments I can design and test against, rather than four adjectives or four arbitrary maximums."`
      },
      {
        id: 'rac-a3',
        type: 'followup',
        ...
        timeboxSec: 120,
        rubric: `Must-haves: (1) vertical scaling correctly defined as making one machine bigger, with its benefit named - simplicity and easier strong consistency with one authoritative copy; (2) horizontal scaling correctly defined as adding more machines, with its benefit named - removing the single-machine ceiling, not every practical ceiling; (3) at least one real cost of horizontal scaling named explicitly - traffic distribution, partial failure, or coordination and partitioning for stateful work; (4) distinguishes stateless services, which scale out simply, from stateful systems, which can scale out with more coordination; (5) gives a concrete case for still scaling up, tied to a reason rather than asserted (moderate, slowly growing load; a single-writer database wanting simple transactions; avoiding distributed complexity before it is needed); (6) does not claim horizontal scaling is always superior or that vertical scaling is only for beginners. Common mistakes: describing only one option; calling horizontal scaling limitless or stateless-only; asserting a preference with no stated reason; forgetting that vertical scaling has a single-machine ceiling. {{HONESTY}}`,
        model: `"Scaling up means making one machine bigger - more CPU, more memory, a faster disk. It is simple: little about the software changes, and one authoritative copy makes strong consistency easier to provide. The costs are a single-machine ceiling and a failure domain that still needs a standby or recovery plan.

Scaling out means adding machines that each carry a share of the load. It removes the single-machine ceiling, not every ceiling. A stateless service is the easy case: a load balancer can send any request to any replica. A stateful database or stream processor can scale out too, but it must partition or replicate data and handle coordination, partial failure and consistency across machines.

Yes, I'd still scale up when measured load is comfortably inside a larger machine and growth is modest. A single-writer relational database is a good example: keeping one primary and scaling it vertically preserves simple local transactions for a long time. I would shard only when write throughput or data volume approaches that machine's practical limit. If availability requires another machine sooner, I would add a standby or replica for failover without pretending that this removes the primary's write ceiling."`
      }
    ],
    sayQuestion: null,
    sayItOutLoud: null,
...
<li><b>HTTP method (verb).</b> GET, POST, PUT, PATCH, DELETE - the standard vocabulary for what a request does to a resource. PATCH (changes only the fields included in the request, unlike PUT, which replaces the whole resource) is the one whose behaviour is least obvious from its name.</li>
...
<li><b>gRPC.</b> A binary RPC (remote procedure call - the caller invokes a function on another machine as if it were local) framework built on HTTP/2 and Protocol Buffers, typically used between a company's own services.</li>
...
<p>A useful rule of thumb: public and browser-facing, reach for REST; two backend services you own calling each other with a low latency budget, reach for gRPC; one endpoint feeding several very different client views of the same data, reach for GraphQL.</p>`,
      ...
      },
      {
        ...
        body: `<p>Only TCP, UDP and QUIC in this section are transports at <b>layer 4</b>, where bytes move between an address and a port. HTTP/2, HTTP/3 and WebSocket are application protocols at <b>layer 7</b>, where the message itself is visible (see <a href="#networking-basics/nb-layers">The network layers, all seven of them</a>). Keeping those two jobs apart is what makes the load-balancing choice later in this chapter a real choice rather than a pair of labels.</p>
<p><b>Transport recap.</b> TCP provides one reliable, ordered byte stream, so a lost packet holds back later bytes on that connection. UDP sends independent datagrams without built-in delivery or ordering guarantees. QUIC runs over UDP and provides reliable streams with loss recovery per stream. The full handshake, loss and trade-off explanation is in <a href="#networking-basics/nb-f2">TCP and UDP: what a connection costs and what it buys</a>.</p>
<p><b>Application-protocol recap.</b> HTTP/2 multiplexes logical request streams over one TCP connection, so TCP loss can still stall them together. HTTP/3 carries those streams over QUIC, limiting that transport-level stall to the affected stream. The complete version comparison is in <a href="#networking-basics/nb-f3">HTTP: requests, responses, and three versions of one protocol</a>.</p>
...
<p>A <b>load balancer</b> answers "which healthy replica of this one service should handle this request?" It operates at <a href="#networking-basics/nb-layers">layer 4</a> (address and port only) or layer 7 (the request itself) and its job is traffic distribution and health-aware routing across a set of interchangeable replicas.</p>
...
<p><b>What is safe to retry.</b> Safe and idempotent HTTP methods make repeated intended effects easier to reason about, but the method name alone is not permission to retry every failure. Retry only a transient or ambiguous failure, within a deadline and retry budget, when the caller knows repeating the operation is safe for this API. A non-idempotent operation (a plain POST, like "charge this card") needs an <b>idempotency key</b> - a value the caller generates once and attaches to every attempt of that logical operation - plus durable server-side state that lets a retry return or reconcile the original attempt instead of blindly doing the work again.</p>
...
<p><b>Hedged requests</b> are one direct mitigation for safe, idempotent reads: send the request to one replica, and only if it has not returned by a measured tail threshold - for example a high percentile chosen from that request class's latency distribution - send one duplicate to an independent replica. Use the first answer and immediately cancel or ignore the other. The threshold and a hedge budget matter: hedging at p50 would duplicate roughly half of ordinary requests, which is not modest load and can make the tail worse. Record the actual extra request rate, cap it, and never hedge a write merely because it is slow.</p>
    ...
    activities: [
      {
        ...
        timeboxSec: 1200,
        rubric: `Marking guide, out of 10. (1) Identifies that a charge is not naturally idempotent and that a timeout leaves an ambiguous result. (2) Creates one durable payment-attempt or order id before calling the payment provider and reuses it for every attempt of that logical charge. (3) Records the key, request fingerprint and local attempt state atomically, without claiming a local database transaction can include an external card-network charge. (4) Passes the same key to a provider that supports idempotency, or names reconciliation/status lookup as necessary when it does not. (5) Distinguishes a service retry from the customer clicking pay again, and explains how the client or checkout service recovers the existing durable attempt id rather than inventing a fresh one. (6) Handles an ambiguous timeout as pending/unknown until a provider status check or reconciliation resolves it, not as a definite failure. (7) Uses a deadline plus bounded exponential backoff with jitter only for retryable outcomes. (8) Returns the stored terminal result on a duplicate with the same key and rejects reuse of that key with different charge details. (9) Names monitoring and reconciliation for attempts stuck in an unknown state. (10) Stays concrete rather than promising exactly-once behavior from one database write. Deduct for: claiming the idempotency row and an external charge are one atomic transaction; blindly retrying; generating a new key for the same customer resubmission; promising a customer can never be double charged without stating the provider and reconciliation boundary. {{HONESTY}}`,
        model: `"A timeout is ambiguous: the provider may never have received the charge, may still be processing it, or may have charged the card and lost only the response. I must not turn silence into a second charge.

Before the external call, I would create a durable payment-attempt id tied to this order and store the request details and a pending state in one local transaction. Every service retry uses that same id as the provider's idempotency key. If the provider supports idempotency, a repeated request returns the first attempt's result. My local transaction cannot include the provider's external side effect, so I still need a state machine: pending, succeeded, failed, or unknown.

If the call times out, I keep the attempt unknown and query the provider by that key or let reconciliation resolve it; I do not report a definite failure and immediately charge again. Retries stay inside the checkout deadline, use bounded backoff with jitter, and apply only to retryable outcomes.

If the customer's client times out and they click pay again, checkout looks up the existing attempt for that order and reuses its key. That key must survive a page reload or another checkout replica. Reusing it with different amount or currency is rejected.

Once the provider result is known, I store the terminal result and return it for future duplicates. I would monitor unknown attempts and reconcile them against provider records, because idempotency narrows the failure window; it does not make my database and a card network one atomic system."`
      },
      {
        id: 'apc-a2',
        type: 'formulate',
        title: 'Choose a load-balancing algorithm for a stateful WebSocket service',
        prompt: `A chat service keeps a persistent WebSocket connection per user, and each replica holds in-memory state only for its currently open connections. Which load-balancing algorithm would you use for new connections, and when would you choose <a href="#networking-basics/nb-layers">layer 4 or layer 7</a>? State what happens to connection state on reconnect, and justify any affinity you add.`,
        timeboxSec: 300,
        rubric: `Must-haves: (1) recognises that long-lived connections make current connection count or reported load important, so least-connections or load-aware placement is a valid default; (2) states that an established WebSocket remains pinned to its chosen replica for that connection's lifetime; (3) does not infer reconnect affinity merely from per-connection memory, because that state is normally gone when the connection dies; durable reconnect state must live in shared storage or be reconstructible; (4) chooses L7 when the balancer must terminate TLS, authenticate the HTTP upgrade, or hash a request-level identity, and accepts L4 as valid for TLS pass-through when the backend performs those jobs; (5) uses consistent hashing or another affinity rule only when a stated requirement makes returning to a replica useful, and names its skew/failure trade-off; (6) explains what happens when a replica fails and why already-open connections must reconnect. Common mistakes: requiring consistent hashing without durable state to recover; declaring L7 universally superior; using round robin without considering uneven connection lifetime; pretending open connections can be moved transparently. {{HONESTY}}`,
        model: `"I would start with least-connections or a backend-reported load measure, because these connections are long-lived and their lifetimes vary. Round robin spreads connection starts, but it can leave one replica holding far more live sockets than another. Once a WebSocket is established, that connection stays pinned to the selected replica until it closes.

Layer 7 is useful if the balancer terminates TLS and must authenticate the HTTP upgrade or route on a user or session id. Layer 4 is also valid when I want TLS pass-through and the backend handles authentication; it can forward the established connection without understanding the HTTP request. I would choose from those requirements, not from WebSocket alone.

I would not add consistent-hash affinity merely because each replica keeps state for its open sockets. When a socket dies, that per-connection state is normally gone. Reconnectable state has to be reconstructed or stored in a shared durable store. If there is a measured benefit to returning a user to the same healthy replica, such as a warm but disposable local cache, I could hash the user id, while accepting skew and remapping on failure.

If a replica fails, its open connections fail too and clients reconnect through the balancer. The design must tolerate that explicitly; a routing algorithm cannot move an already-open TCP connection or recover memory that died with its replica."`
      },
      {
        id: 'apc-a3',
        type: 'followup',
